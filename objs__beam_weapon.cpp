...
static void draw_sprite(const i4_3d_vector &p, r1_texture_handle tex, float sprite_scale)
{
   ...
   */
    r1_clip_render_textured_rect((cx-w/2), (cy-w/2), 
                                 (cx+w/2), (cy+w/2), p.z,
                                 1.0f,
                                 i4_f_to_i(g1_render.center_x*2),
                                 i4_f_to_i(g1_render.center_y*2),
...
void g1_beam_weapon_class::draw(g1_draw_context_class *context, i4_3d_vector& viewer_position)
{
  if (should_draw)
  {
    ...
    float theta=g1_tick_counter*3.0f + fr;
    float theta_step=2*i4_pi()/10.0f;

    
    for (float t=0; t<1; t+=(1.1f-t)*0.3f)
    {
      i4_3d_vector s=i4_3d_vector(start.x + step.x*t + side.x*(float)cos(theta),
                                  start.y + step.y*t + side.y*(float)sin(theta),
                                  start.z + step.z*t + side.z*(float)cos(theta+0.5)*0.5f);
    
      theta+=theta_step;
      
      context->transform->transform(s, t_start);            
      draw_sprite(t_start, texture, 0.1f);
    }
  }
}
