			 ...
			 i4_bool clip_code_and_project_done)
{
	...
	w32 i,j,c0,c1;
	w32 bitmask;
	i4_float ooz,dx,dy,dz,ds,dt,dr,dg,db,da,t=0;
						  ...
						  r1_render_api_class * api)
{
	...
	for (i=0; i<t_lines; i++)
	{
		...
		if (verts[i].outcode & verts[i+1].outcode)
		{
			continue;
		}
								  ...
								  float s2, float t2)
{
	...
	if (v[1].px < clip_x1)
	{
		...
		v[1].s = s1+(s2-s1)*diff;
		v[2].s = s1+(s2-s1)*diff;
		v[1].px = (float)clip_x1;
		v[2].px = (float)clip_x1;
	}
	...
	if (v[0].px > clip_x2)
	{
		...
		v[0].s = s2 - diff*(s2-s1);
		v[3].s = s2 - diff*(s2-s1);
		v[0].px = (float)clip_x2;
		v[3].px = (float)clip_x2;
	}
	...
	if (v[0].py < clip_y1)
	{
		...
		v[0].t  = t1+(t2-t1)*diff;
		v[1].t  = t1+(t2-t1)*diff;
		v[0].py = (float)clip_y1;
		v[1].py = (float)clip_y1;
	}
	...
	if (v[2].py > clip_y2)
	{
		...
		v[2].t  = t2-diff*(t2-t1);
		v[3].t  = t2-diff*(t2-t1);
		v[2].py = (float)clip_y2;
		v[3].py = (float)clip_y2;
	}
