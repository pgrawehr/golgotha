...
const i4_float FUNNEL_HEIGHT=7.0f;
const i4_float FUNNEL_RADIUS=1.0f;
const int DATA_VERSION=1;
const int MAX_VALS=256;
const i4_float RADIUS=0.01f;
static i4_float rand_val[MAX_VALS];
static int rand_i;
inline i4_float fast_rand()
{
	rand_i=(rand_i+1)&(MAX_VALS-1);
	return rand_val[rand_i];
}
...
void g1_debris_class::setup(i4_float sx, i4_float sy, i4_float sz, int _type)
{
	...
	if (sx>map->width()-1)
	{
		sx=(float)(map->width()-1);
	}
	if (sy>map->height()-1)
	{
		sy=(float)(map->height()-1);
	}
	...
	switch (type)
	{
		case TORNADO:
			...
			{
				debris[i].x = g1_float_rand(i)*FUNNEL_RADIUS*2.0f-FUNNEL_RADIUS;
				debris[i].y = g1_float_rand(i+2)*FUNNEL_RADIUS*2.0f-FUNNEL_RADIUS;
		...
		case RING_CHUNKS:
			for (i=0; i<MAX_DEBRIS; i++)
			{
				debris[i].x = g1_float_rand(i+4)*rad*2.0f-rad;
				debris[i].y = g1_float_rand(i+6)*rad*2.0f-rad;
...
void g1_debris_class::think()
{
	i4_float
	spin_speed = 0.3f,
	cs = (float)cos(spin_speed)*1.05f,
	sn = (float)sin(spin_speed)*1.05f;
		...
		case TORNADO:
			for (i=0; i<MAX_DEBRIS; i++)
			{
				...
				if (debris[i].z>FUNNEL_HEIGHT)
				{
					...
					debris[i].z = g1_float_rand(i+2)*0.3f;
					ldebris[i] = debris[i];
				}
			}
			break;
		case RING_CHUNKS:
			...
			for (i=0; i<MAX_DEBRIS; i++)
			{
				...
				if (debris[i].z>strength)
				{
					...
					debris[i].x = (float)cos(th)*rad;
					debris[i].y = (float)sin(th)*rad;
					debris[i].z = g1_float_rand(i+8)*0.2f;
					ldebris[i] = debris[i];
				}
			}
			break;
