...
void g1_input_class::receive_event(i4_event * ev)
{
	if (ev->type()==i4_event::DO_COMMAND)
	{
		...
		{
			g1_human->build_unit(g1_get_object_type(kev->command+6));
		}                                                               // build an object
		else
		{
			switch (com)
			{
				case G1_MAIN_MENU:
					esc=i4_T;
					break;
				case G1_TOGGLE_STATS:
					g1_stat_counter.show();
					break;
				case G1_SHOW_HELP:
					i4_kernel.send_event(i4_current_app, ev);
					break;
			}
		}
	}
	...
	else if (ev->type()==i4_event::KEY_PRESS)
	{
		...
		cheat_code_buffer[0] = (char)kev->key_code;
	}
