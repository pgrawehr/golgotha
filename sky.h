...
class li_g1_sky:public li_object
	{
	friend class li_g1_sky_function;
	protected:
		li_string *name;
	public:
		li_g1_sky(li_string *_name):li_object(li_g1_sky_type),
			name(_name){}

		li_g1_sky(const i4_const_str &str):li_object(li_g1_sky_type)
			{
			name=new li_string(str);
			}

		li_g1_sky(i4_const_str &str):li_object(li_g1_sky_type)
			{
			name=new li_string(str);
			}
		li_g1_sky(const char *str):li_object(li_g1_sky_type)
			{
			name=new li_string(str);
			}
		li_string *value(){return name;}
...
public:
  ...
  {
    v.px=x*(width());  v.py=y*(height());  v.v.z=50.0f;  v.w=1.0f/v.v.z;
    v.s=x; v.t=y;
    v.r=v.g=v.b=1.0f;
    v.a=1;                                                
  }
