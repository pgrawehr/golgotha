...
//  w32 i;
	...
	else
	{
		...
		v3_1=i4_3d_vector(v1.x - v3.x, v1.y - v3.y,  0);
		v3_1.normalize();
		v2_0=i4_3d_vector(v0.x - v2.x, v0.y - v2.y, 0);
		v2_0.normalize();
...
//   }
	...
	if (get_flag(DRAGGING)&&view.get_view_mode()!=G1_EDIT_MODE) //already drawing the rect if in edit mode
	{
	...
	if (g1_human)
	{
		...
		//i4_float s_x,s_y,s_h;

		//this definitelly needs a change.
		//r1_vert v;
		//s_x=g1_human->selected_object->x;
		//s_y=g1_human->selected_object->y;
		//s_h=g1_human->selected_object->h;
		//if (g1_render.project_point(i4_3d_vector(s_x,s_y,s_h),v,context->transform))
		//	  {
		//	  draw_spining_tris(v.px,v.py,v.v.z,
		//		  (g1_tick_counter + g1_render.frame_ratio)/4.0f,
		//		  lock_own.get(),
		//		  0);
		//	  }
	}
