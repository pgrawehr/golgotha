...
#include "render/glide/gr_vram.h"
#include "image/image.h"
#include "render/r1_api.h"
#include "time/profile.h"
#include "video/glide/glide_display.h"
...
#ifdef _WINDOWS
#include "render/software/amd3d/amd3d.h"
#define USE_AMD3D
#endif
...
static struct gr_static_struct
{
	...
	0.f, 0.f, 0.f, 0.f, 1.f, 1.f, 255.f, 1.f, (float)(1L<<19), (float)(1L<<19), 1.f
};
...
class r1_glide_render_window_class :
...
public:
	...
	{
		if (num_color_tints==MAX_COLOR_TINTS)
		{
			return 0;
		}
	...
	r1_glide_vram_class *vram()
	{
		return (r1_glide_vram_class *)tmanager;
	}
	...
	virtual void set_write_mode(r1_write_mask_type mask)
	{
		...
		{
			// need to change the color write mask
			grColorMask(mask & R1_WRITE_COLOR ? 1 : 0, 0);
	...
	void clear_area(int x1, int y1, int x2, int y2, w32 color, float z)
	{
		...
		if (!gr_copy_vert_function(g_vert_buf, v, 4))
		{
			return;
		}
...
looper:
		cmp esi,1
		je dont_prefetch prefetch(_eax)
		prefetch(_ebx)

		dont_prefetch :

			movq mm3,
			qword ptr [edi] r1_vert.a   //src->w  | src->a
			movq mm2,
			qword ptr [edi] r1_vert.r   //src->g  | src->r

			movq mm1,
			qword ptr [edi] r1_vert.s   //src->t  | src->s
			movq mm0,
			qword ptr [edi] r1_vert.px  //src->py | src->px
			...
			movq qword ptr [edx] GrVertex.a,
			mm3                         //dst->oow = src->w*gr_w_mul | dst->a = src->a * 255.f
			punpckhdq mm3,
			mm3       //mm3 - oow | oow

			movq qword ptr [edx] GrVertex.r,
			mm2                         //dst->r = src->r*gr_r_mul | dst->g = src->g * gr_g_mul
			add eax,
			dword ptr [r1vtxsize]

			movd dword ptr [edx] GrVertex.tmuvtx[0].oow,
			mm3 pfmul(m1, m3) //mm1 - tow | sow

			movq mm3,
			qword ptr [ecx] gr_static_struct.VTX_SNAP_1
			add ebx,
			dword ptr [grvtxsize]

			movd mm2,
			dword ptr [edi] r1_vert.b   //mm2 - src->b
			movq qword ptr [edx] GrVertex.tmuvtx[0].sow,
			mm1                                    //dst->tmuvtx[0].sow = src->s*src->oow;
												   //dst->tmuvtx[0].tow = src->t*src->tow

			pfsub(m0,m3) //mm0 - src->py + gr_yoff | src->px + gr_xoff
			movd mm3,
			dword ptr [ecx] gr_static_struct.gr_b_tint_mul  //mm3 - gr_b_mul

			movq qword ptr [edx] GrVertex.x,
			mm0                        //dst->x = src->px+VTX_SNAP-VTX_SNAP+gr_x_off
			...
			movd dword ptr [edx] GrVertex.b,
			mm2                         //dst->b = src->b * gr_b_mul
			add edx,
			dword ptr [grvtxsize]
