...
void g1_player_piece_class::skipload(g1_loader_class *fp)
{
	...
	if (fp && fp->check_version(AMMO_DATA_VERSION))
	{
		for (int i=0; i<MAX_WEAPONS; i++)
		{
			ammo[MAX_WEAPONS].load(fp);
		}                              //just ignore what's there... (we have added an additional entry)
		fp->end_version(I4_LF);
	}
											...
											i4_float braking_friction)
{
	...
	if (t>speed*speed+0.0025 || braking_friction==0.0)
	{
		...
		{
			stop_dist = (float)fabs(speed)/braking_friction;
		}                                              // geometric series of braking motion
...
//          i4_float dist, dx,dy;
	...
	else if (x>g1_get_map()->width()-2)
	{
		x=(float)g1_get_map()->width()-2;
	}
	if (y<1)
	{
		y=1;
	}
	else if (y>g1_get_map()->height()-2)
	{
...
//    i4_float dist, dx,dy;
	...
	if (on_base)
	{
		...
		if (health>ammo[3].ammo_type->max_amount)
		{
			...
			if (refuel_handle.playing)
			{
				s1_end_looping(refuel_handle);
			}
		}
		else
		{
...
void g1_player_piece_class::think()
{
	...
	if (health>ammo[3].ammo_type->max_amount)
	{
		//since the static defaults->health
		health=ammo[3].ammo_type->max_amount;
	}                                      //of an stank is very high.
...
//     }
	...
	else
	{
		...
		if (!get_stank_flag(ST_INFLIGHT))
		{
			...
			if (strafe_accel_ratio<0)
			{
				if (accel_ratio<0)
				{
					desired_base = 3 *i4_pi()/4+base_angle;
				}
				else if (accel_ratio>0)
				{
					desired_base=i4_pi()/4+base_angle;
				}
				else
				{
					desired_base=i4_pi()/2+base_angle;
				}
			...
			else if (strafe_accel_ratio>0)
			{
				if (accel_ratio<0)
				{
					desired_base=5 *i4_pi()/4+base_angle;
				}
				else if (accel_ratio>0)
				{
					desired_base=7 *i4_pi()/4+base_angle;
				}
				else
				{
					desired_base=3 *i4_pi()/2+base_angle;
				}
	...
	switch ((int)(base_angle*8/(2*i4_pi())))
	{
		case 0:
			...
			break;
		case 1:
			radar_image=&radar_im2;
			break;
		case 2:
			radar_image=&radar_im4;
			break;
		case 3:
			radar_image=&radar_im6;
			break;
		case 4:
			radar_image=&radar_im8;
			break;
		case 5:
			radar_image=&radar_im10;
			break;
		case 6:
			radar_image=&radar_im12;
			break;
		case 7:
			radar_image=&radar_im14;
			break;
	}
	...
	if (player_num==g1_player_man.local_player)
	{
		g1_unfog_radius(i4_3d_vector(x,y,h), 7.0);
	}                                         //Only unfog for the local stank
...
//  {
	for (int i=0; i<MAX_WEAPONS-1; i++)
	{
		if (fire[i])
		{
			...
			{
				fire_weapon(i);
			}              //don't fire with remote stanks. They will send us info about
			//bullets.
		}
	}
//  }
