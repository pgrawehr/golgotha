...
(defvar sentence
  '((how can you say that (statement) \?)
    ...
    (No one calls (person) a (der-term) \!)
    (man\, (sentence))))
...
;;used kinda strange syntax (`(x...)) is only `(x...) in clisp
(defun define-element (name)
  (let ((loop-to-use (intern (concat name "-loop"))))
    (eval `(defun ,(intern name) nil
...
(defun flatten (plist)
  (cond ((atom plist)
	 (list plist))
	(t (apply 'append (mapcar 'flatten plist)))))
...
(defun flames (arg) 
  (cond ((zerop arg) t)
	(t (progn (print (flatten (*flame))) (flames (1- arg))))
	))

(defun flame (&optional (arg 1))
