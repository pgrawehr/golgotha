...
public:
  ...
  g1_human_def_class()
    : g1_team_api_definition_class("human") {g1_human_class_references=0;}
...
void g1_human_class::attack_unit(g1_object_class *o,i4_float x, i4_float y)
	{
	if (!selected_object.valid()) return;
...
class special_command_entry_cleanup_class:public i4_init_class
	{
	void uninit()
		{
...
//		}
  ...
  else
  {
	...
	if (g1_current_controller->view.get_view_mode()==G1_FOLLOW_MODE)
		...
		if (g1_resources.follow_camera_dist<=0.5f) g1_resources.follow_camera_dist=0.5f;
