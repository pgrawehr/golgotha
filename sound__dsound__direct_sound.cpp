...
void dsound_buffer_class::set_pan(i4_pan pan)
{
	if (!pDSB || p3DSB)
	{
		//dont call set_pan on 3d sounds, dummy
		return;
	}
...
i4_bool direct_sound_class::setup()
{
	...
	if (i4_win32_startup_options.use3dsound ==TRUE)
	{
		use_3d_sound=i4_T;
	}
	else
...
void dsound_buffer_class::set_3d_position(i4_float x, i4_float y, i4_float z, i4_bool immediately)
{
	if (p3DSB)
	{
		...
		if (immediately)
		{
			apply_time = DS3D_IMMEDIATE;
		}
		else
		{
			apply_time = DS3D_DEFERRED;
		}
...
i4_voice_class *direct_sound_class::alloc(w32 buffer_size, sound_parameters &desc)
{
	...
	}                              //We won't be able to restore the buffer.
...
li_object *setsound(li_object *o, li_environment *env)
{
	...
	if (li_is_number(li_first(o,env),env))
	{
		...
		if (set>=1)
		{
			i4_direct_sound_class_instance.enable_sound();
		}
		else
		{
			i4_direct_sound_class_instance.disable_sound();
		}
