...
i4_float grade_max[G1_GRADE_LEVELS] = {
	10, 17, 30, 256
};
...
void g1_map_class::recalc_static_stuff()
{
	if (recalc==0)
	{
		//don't do anything if recalc is zero.
		return;
	}
		...
		for (int y=1; y<(int)h-1; y++)
		{
			for (int x=1; x<(int)w-1; x++, c++, v++)
			{
				if ((g1_tile_man.get(c[0].type)->flags & g1_tile_class::WAVE) &&
					...
					(g1_tile_man.get(c[-(int)w-1].type)->flags & g1_tile_class::WAVE))
				{
					// JJ cast
	...
	{
		tick_time.get();
	}                    // don't simulate ticks for the calculation stuff
}
...
//{{{
{
	...
	right  = i4_f_to_i(x + range);
	if (right>wx-1)
	{
		right=wx-1;
	}
	top    = i4_f_to_i(y - range);
	if (top<0)
	{
		top=0;
	}
	bottom = i4_f_to_i(y + range);
	if (bottom>wy-1)
	{
		bottom=wy-1;
	}
										   ...
										   OBJ_FILTER_TYPE fn) const
{
	...
	x_left   = i4_f_to_i(x - range);
	if (x_left<0)
	{
		x_left=0;
	}
	x_right  = i4_f_to_i(x + range);
	if (x_right>(sw32)(w-1))
	{
		x_right=w-1;
	}
	y_top    = i4_f_to_i(y - range);
	if (y_top<0)
	{
		y_top=0;
	}
	y_bottom = i4_f_to_i(y + range);
