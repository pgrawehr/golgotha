										  ...
										  w32 newbitdepth, i4_change_type change_type)
{
	...
	{
		return i4_F;
	}                                                    //Not yet initialized, can't continue
...
void i4_dx5_display_class::flush()
{
	...
	else
	{
		...
		r.bottom=wh;
		//do a signed compare
		if ((r.bottom+pt.y)>(int)pdesc.dwHeight)
		{
			r.bottom=pdesc.dwHeight-pt.y;
		}                                                                     //Bltfast doesn't like to copy things outside screen.
		r.right=ww;
		if ((r.right+pt.x)>(int)pdesc.dwWidth)
		{
			r.right=pdesc.dwWidth-pt.x;
		}
		// JJ
		// 0  // 0
		HRESULT prob=0;
...
void i4_dx5_display_class::FlipToGDISurface()
{
	if (use_page_flip)
	{
		...
		if (dx5_common.ddraw)
		{
			dx5_common.ddraw->FlipToGDISurface();
		}
	}
	return;
}
...
class li_add_null_gdi_surface_class :
