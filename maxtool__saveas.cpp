...
li_object *m1_save(li_object * o, li_environment * env)
{
	if (m1_info.obj && m1_info.models.size()>m1_info.current_model)
	{
		...
		if (fp)
		{
			...
			m1_info.obj->save(sfp);
			delete sfp; //fp gets deleted, too, because the second parameter
			//of i4_saver_class (wheter to delete fp) defaults to true.
		}
	}
	return 0;
}
