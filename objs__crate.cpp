...
static g1_model_ref health_model("crate_health"),
...
S1_SFX(missile_sfx, "misc/missile_powerup_22khz.wav", 0, 200);
S1_SFX(chain_sfx, "misc/supertank_chain_gun_refuel.wav", 0, 200);
S1_SFX(money_sfx, "misc/powerup_money_22khz.wav", 0, 200);


float &g1_crate_class::yvel()
{
	return vars->get(li_yvel);
}
int &g1_crate_class::ticks_left()
{
	return vars->get(li_ticks_left);
}
...
void g1_crate_class::note_stank_near(g1_player_piece_class * s)
{
	...
	if (dist_sqrd<0.5*0.5)
	{
		...
		switch (get_ctype())
		{
			...
			default:
				//go_away();
				sprintf(msg, "The crate contains something unusable.");
				break;
		...
		//otherwise, we cannot use more of what's inside the crate
		//(i.e we have full health)
...
//   }
	...
	set_ctype(get_ctype());   // to setup the draw params
