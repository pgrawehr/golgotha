...
{  
  ...
  else
  {
    ...
    if (!p) 
		{
		i4_error("INTERNAL: ~s1_sfx_ref(): object not found");
		return;
		}
    last->next=next;
  }
...
void s1_unload()
{
  ...
  for (int i=0; i<T_CHANNELS; i++)
  {
    channels[i].free();//free channels if not already done.
    channels[i].voice=0;
    channels[i].stream=0;
    channels[i].loop_handler=0;
    channels[i].originator=0;
...
int find_channel(s1_sfx_ref *ref, s1_sound_handle *h)
{
  ...
  if (!channels_sorted)
  {
	...
	for (i=0;i<T_CHANNELS;i++)
		{
		if (channels[i].loop_handler)
			{
			channels[i].loop_handler->channel_on=i;
			//place correct channel number
			}
		}
  }
...
void s1_end_looping(s1_sound_handle &handle)
{
  ...
  else
  {
    ...
    i4_error("INTERNAL: s1_end_looping(): sfx not in list.");
  }
