...
void g1_resource_class::g_point(i4_3d_point_class &p, char * res_name)
{
	...
	p.x=(float)i.read_float();

	x++;
	i=x->begin();
	p.y=(float)i.read_float();

	x++;
	i=x->begin();
	p.z=(float)i.read_float();
...
void g1_resource_class::g_rect(rect &r, char * res_name)
{
	...
	s=x;
	i4_const_str::iterator i=x->begin();
	r.x1=(short)i.read_number();

	x++;
	i=x->begin();
	r.y1=(short)i.read_number();

	x++;
	i=x->begin();
	r.x2=(short)i.read_number();

	x++;
	i=x->begin();
	r.y2=(short)i.read_number();
...
void g1_resource_class::load()
{
	...
	try{
		...
		damping_fraction = 1.0f/damping_friction;
		ln_damping_friction = (float)exp(0.1f*damping_friction);
		...
		{
			i4_error("no build_context_help in resource/g1.res");
		}
	...
	{
		i4_error("FATAL: Number format exception reading default resources 'g1.res'.");
	}
}
