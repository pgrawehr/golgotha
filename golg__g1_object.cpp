...
g1_object_definition_class::g1_object_definition_class(char *_name,
  ...
  : _name(_name),
    init_function(_init), uninit_function(_uninit),special_init_function(0)
{
  flags=type_flags;
  defaults=0;
...
g1_object_class::g1_object_class(g1_object_type id,
  ...
  : id(id), occupied_squares(4,0), vars(0), world_transform(0),
    mini_objects(0), num_mini_objects(0), ref_list()
{
  w16 ver=0;
  w16 data_size=0;
  ...
  for (i=0; i<model->num_vertex; i++)
  {
    ...
    if ((cell->flags & g1_map_cell_class::SCRATCH1)==0)
    {
      ...
      if ( ++num_squares > (MAX_SQUARES - 2))
		  break;//just leave them unoccupied.
    }
    vert++;
  }
...
class g1_object_marker_class : public li_gc_object_marker_class
{
public:
