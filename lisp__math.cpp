...
li_bignum *li_make_bignum_d(double in)
	...
	if (fabs(in)<0.5) return li_bigzero;
	//char signum=in<0;
	...
	char *dot=strchr(buf,'.');
	if (dot) (*dot)=0x0; //set a 0 where the point was
...
li_bignum *li_make_bignum(li_object *o,li_environment *env)
	...
	if (o->type()==LI_BIGNUM) return li_bignum::get(o,env);
	if (o->type()==LI_INT) return li_make_bignum(li_int::get(o,env)->value());
	if (o->type()==LI_FLOAT) return li_make_bignum_d(li_float::get(o,env)->value());
...
i4_bool li_greater_bignum(li_bignum *a, li_bignum *b)
	...
	if (a->get_signum()&&b->get_signum()) signinverse=i4_F;
	if (a->get_signum()&& !b->get_signum()) return i4_F;
	if (!a->get_signum() && b->get_signum()) return i4_T;
	char *bbuf=b->value();
	...
	if (alen-astart>blen-bstart) return signinverse;
	if (alen-astart<blen-bstart) return !signinverse;
	while(astart<=alen&&bstart<=blen)
		{
		if ((*abuf)>(*bbuf)) return signinverse;
		if ((*abuf++)<(*bbuf++)) return !signinverse;
...
li_bignum *li_sub_bignum(li_bignum *a, li_bignum *b)
	...
	do
		...
		bval=bbuf>=b->value()?(*(bbuf--)):0;
		bval+=curval;
		if (bval>aval) {curval=0x100+(aval+10-bval);}
		else curval=aval-bval;
		buf[len]=(w8)curval;
		curval>>=8;
		len--;
		}
...
//returns a bignum if needed, an int otherwise
	...
	if (len>10) return in;
	  while (len)
		  {
		  if (res>(0x7fffffff-10)) return in;
		  res=res*10+in->value()[len];
		  len--;
		  }
	  if (in->get_signum()) res= -res;
	  return new li_int(res);
	}

class li_math_buffer_class:public i4_init_class
	{
	public:
		char *buffer;
		w32 length;
		li_math_buffer_class(){buffer=0;length=0;}
...
li_object *li_subtract(li_object *o,li_environment *env)
	...
	if (o1->type()==LI_INT)
		...
		int summe=li_get_int(o1,env);
		li_object *o2=li_cdr(o,env);
		if (!o2) return new li_int(-summe);// - with one arg means complement
		while (o2)
			{
			summe-=li_get_int(li_eval(li_car(o2,env),env),env);
			o2=li_cdr(o2,env);
		...
		double summe=li_get_float(o1,env);
		li_object *o3=li_cdr(o,env);
		if (!o3) return new li_float(-summe);
		while(o3)
			{
			summe-=li_get_float(li_eval(li_car(o3,env),env),env);
			o3=li_cdr(o3,env);
...
li_object *li_read_number_in_radix(int rd,char *tk)
	...
	while(*tk);
	if (b)
		return li_copy_bignum(b,!sig);//uses inverse value for sig
...
class li_math_register_class:public i4_init_class
	{
	virtual void init()
		{
	...
	virtual int init_type(){return I4_INIT_TYPE_LISP_FUNCTIONS;}
