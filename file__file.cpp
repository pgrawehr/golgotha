...
int i4_file_class::write_format(char *format, ...)
{
	...
	while (*f)
	{
		...
		{
			case '1':
				write_8(*(va_arg(ap,     w8 *)));
				break;
			case '2':
				write_16(*(va_arg(ap,    w16 *)));
				break;
			case '4':
				write_32(*(va_arg(ap,     w32 *)));
				break;
			case 'f':
				write_float(*(va_arg(ap,  float *)));
				break;
			case 'S':
				write_counted_str(  *(va_arg(ap, i4_const_str *)));
				break;
		}
		f++;
	}
		...
		switch (*f)
		{
			case '1':
				*(va_arg(ap,   w8 *))=read_8();
				break;
			case '2':
				*(va_arg(ap,  w16 *))=read_16();
				break;
			case '4':
				*(va_arg(ap,  w32 *))=read_32();
				break;
			case 'f':
				*(va_arg(ap, float *))=read_float();
				break;
			case 'S':
				*(va_arg(ap, i4_str **))=read_counted_str();
				break;
		}
		f++;
	}
...
w32 i4_sub_section_file::read(void *buffer, w32 rsize)
{
	...
	{
		// don't allow reads past the sub section
...
class i4_win32_async_priority_reader :
	public i4_win32_async_reader
{
public:
	w32 max_priority()
	{
...
enum {
	S_IRWUGO=(S_IRUSR|S_IRGRP|S_IROTH| S_IWUSR|S_IWGRP|S_IWOTH)
};
enum {
	S_ALL   =(S_IRUSR|S_IRGRP|S_IROTH| S_IWUSR|S_IWGRP|S_IWOTH | S_IXUSR|S_IXGRP|S_IXOTH)
};

class i4_unix_async_reader :
	public i4_async_reader
{
public:
	w32 max_priority()
...
//#ifndef SUN4
class i4_unix_async_priority_reader :
	public i4_unix_async_reader
{
public:
	w32 max_priority()
	{
...
#ifdef _WINDOWS
class i4_win32_file_manager_class :
	public i4_file_manager_class
{
public:
	virtual i4_file_class *open(const i4_const_str &name, w32 flags)
...
class i4_linux_file_manager_class :
	public i4_file_manager_class
{
