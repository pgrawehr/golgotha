...
void g1_explode_model_class::grab_model(g1_object_class * o, i4_3d_vector &obj_center)
{
	...
	center.x=0; //this var seems to be unused anyway.
	center.y=0;
	center.z=0;
	if (!base_m)
	{
	...
	//  while (m)
	{
		...
		for (i=0; i<m->num_quad; i++, f++)
		{
			...
			for (j=0; j<tv; j++)
			{
				// move points relative to center so they spind about it
				v[j].v -= center;
			}
...
void g1_explode_model_class::think()
{
	...
	for (int i=0; i<num_faces; i++)
	{
		...
		if (params.stages[params.current_stage].type==G1_APPLY_SING || (((g1_tick_counter+i)&31)==0))
		{
			...
			float xr=(g1_float_rand(9)-0.5f) *0.2f;
			float yr=(g1_float_rand(3)-0.5f) *0.2f;
			float zr=(g1_float_rand(4)-0.5f) *0.2f;
...
void g1_explode_model_class::draw(g1_draw_context_class * context, i4_3d_vector& viewer_position)
{
	...
	//Suche nach dem Grund, wieso Explosionen nicht transparent sind.
	g1_render.r_api->set_alpha_mode(R1_ALPHA_LINEAR);
	g1_render.r_api->set_constant_color(0x0);
...
{
	stages[0].setup(1,0.01f, G1_APPLY_WHOLE); // apply force for 2 ticks
	stages[1].setup(3, 0,    G1_APPLY_WHOLE); // wait 20 ticks with no force to watch results
	stages[2].setup(2, 1.0f, G1_APPLY_SING); // wait 20 ticks with no force to watch results
