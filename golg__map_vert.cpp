                                          ...
                                          i4_float &height, i4_float &pitch, i4_float &roll)
{
  ...
  for (;chain; chain=chain->next_solid())
		...
		chain->object!=for_obj)
    {
      ...
      {
        height=z+0.2f+ray.z;
        pitch = i4_atan2(normal.x,(float)sqrt(normal.z*normal.z  + normal.y*normal.y));
        roll  = i4_atan2(-normal.y,(float)sqrt(normal.z*normal.z + normal.x*normal.x));
...
i4_float g1_map_class::map_height(i4_float x, i4_float y, i4_float z) const
{
  ...
  if (!(ix>=0 && iy>=0 && ix<(w16)w && iy<(w16)h))
  {
    return 1000;
  }
