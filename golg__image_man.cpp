...
{
  if (im&&im!=default_image)//that one must be handled manually.
	  delete im;
  im=0;
  if (first==this)
    first=first->next;
...
void g1_image_ref::set_image(const char *filename)
{
  ...
  memcpy(image_name,filename,strlen(filename)+1);
...
g1_team_icon_ref::~g1_team_icon_ref()
	{
	cleanup();
	}
