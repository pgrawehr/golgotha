...
void g1_particle_emitter_class::think()
{
	...
	if (t_in_use)
	{
		...
		for (int i=0; i<MAX_PARTICLES; i++)
		{
			if (particles[i].in_use)
			{
				...
				else
				{
					...
					if (particles[i].x<bx1)
					{
						bx1=particles[i].x;
					}
					if (particles[i].x>bx2)
					{
						bx2=particles[i].x;
					}
					if (particles[i].y<by1)
					{
						by1=particles[i].y;
					}
					if (particles[i].y>by2)
					{
						by2=particles[i].y;
					}
				}
		...
		if (bx1!=100000)
		{
			...
			if (bx2>=g1_get_map()->width())
			{
				bx2=(float)g1_get_map()->width()-1;
			}
			if (by1<0)
			{
				by1=0;
			}
			if (by2>=g1_get_map()->height())
			{
				by2=(float)g1_get_map()->height()-1;
			}
