...
static inline int i4_is_black(w32 c)
{
  ...
  if((c&0x00ffffff)==FONT_BACKGROUND) //Das blau der Grafik
    return 1;
  else return 0;
}
...
void r1_font_class::calculatefontdata(i4_image_class *im, int start_ch)
	...
	while (x<im->width()&&(i<126))//This should change if we want to use fonts with extended char sets
		...
		posneu[i].x=startx-1;
		if (posneu[i].x>lw) lw=posneu[i].x;
                                  ...
                                  i4_draw_context_class &context)
{
	  ...
	  char_def_neu chd;h=font->height();
	  chd=posneu[c.value()];
	  ...
	  for (int j=0;j<h;j++)
		  ...
		  for (int i=chd.x;i<(xundw);i++)
			...
			if (color==0xffffff) color=col;//Weiss->Vordergrundfarbe
			//if (c!=0x1492)
  ...
  for (i4_const_str::iterator p=string.begin(); p!=string.end(); ++p)
	  ...
	  for (j=0;j<h;j++)
		  ...
		  for (i=chd.x;i<(xundw);i++)
			...
			if (c==0xffffff) c=col;//Weiss->Vordergrundfarbe
			//if (c!=0x1492)
