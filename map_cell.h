...
public:
	...
	i4_float scratch_len; //used for A* solver
	...
	enum
	{
		...
		VISITED=(1<<7), //These two are for the solver.
		ROUTEOK=(1<<8),
	...
	enum {
		SAVED_FLAGS = FOGGED | MIRRORED | ROTATE_BITS
	};

	void clear_restore_bits()
	{
		flags &= ~(RADAR_NEEDS_STACKED_RESTORE | RADAR_NEEDS_RESTORE);
	}
	i4_bool mirrored()
	{
		return (i4_bool)flags & MIRRORED;
	}
...
//   }
	...
	{
		flags=(flags & (~ROTATE_BITS))|(type<<2);
	}
	g1_rotation_type get_rotation()
	{
		return (g1_rotation_type)((flags&ROTATE_BITS)>>2);
	}

	void init(w16 _type, g1_rotation_type rotation, i4_bool mirrored);

	i4_bool is_blocking() const
	{
		return !(flags&IS_GROUND);
	}
	...
	g1_object_chain_class *get_obj_list()
	{
		return object_list;
	}
