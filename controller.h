...
protected:
	...
	void set_flag(flag_type flag, i4_bool on=i4_T)
	{
		if (on)
		{
			flags|=flag;
	...
	i4_bool get_flag(flag_type flag)
	{
		if (flags & flag)
		{
			return i4_T;
...
public:
	...
	struct spin_event
	{
		...
		spin_event(g1_model_id_type model, w8 type) :
			model(model),
			type((spin_type)type) {
		}
	};
...
public:
	...
	virtual i4_bool clear_if_cannot_draw()
	{
		return i4_T;
	}
...
class g1_map_object_event_class :
