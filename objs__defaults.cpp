...
li_object *g1_defaults(li_object * i, li_environment * env)
{
	while (i)
	{
		...
		d->speed=li_get_float(li_car(o,env),env);
		o=li_cdr(o,env);
		d->turn_speed=li_get_float(li_car(o,env),env);
		o=li_cdr(o,env);
		d->accel=li_get_float(li_car(o,env),env);
		o=li_cdr(o,env);
		d->cost=li_get_int(li_car(o,env),env);
		o=li_cdr(o,env);

		d->health=li_get_int(li_car(o,env),env);
		o=li_cdr(o,env);
		d->fire_delay=li_get_int(li_car(o,env),env);
		o=li_cdr(o,env);
		d->detection_range=li_get_float(li_car(o,env),env);
		o=li_cdr(o,env);

		d->fire_type=li_symbol::get(li_car(o,env),env);
		o=li_cdr(o,env);
...
li_object *g1_def_weapon_damage(li_object * i, li_environment * env)
{
	while (i)
	{
		...
		{
			li_error(env, "USER: Unknown hurt type %O, should be sing or mult", o);
		}
		o=li_cdr(o,env);

		a->default_damage=li_get_int(li_car(o,env),env);
		o=li_cdr(o,env);
		a->special_damage=li_get_int(li_car(o,env),env);
		o=li_cdr(o,env);
		a->speed=li_get_float(li_car(o,env),env);
		o=li_cdr(o,env);
		a->range=li_get_float(li_car(o,env),env);
		o=li_cdr(o,env);
...
void g1_unload_object_defaults()     // call at end of game to clean up memory
{
	...
	{
		if (g1_stank_ammo_types[i].icon)
		{
			delete g1_stank_ammo_types[i].icon;
		}
		if (g1_stank_ammo_types[i].dark_icon)
		{
			delete g1_stank_ammo_types[i].dark_icon;
		}
	}
