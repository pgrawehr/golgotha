...
#include "memory/binary_tree.h"
#include "time/time.h"

class i4_key_man_class :
...
protected:
	...
	struct context
	{
		...
		w16 modifiers_used;
		context() {
			modifiers_used=modifiers_taken=0;
		}
	};
...
public:
	i4_bool control_pressed()
	{
		return (key_modifiers_pressed & I4_MODIFIER_CTRL) ? i4_T : i4_F;
	}
	i4_bool alt_pressed()
	{
		return (key_modifiers_pressed & I4_MODIFIER_ALT) ? i4_T : i4_F;
	}
	i4_bool shift_pressed()
	{
	...
	{
		return (*context_list)[context].name;
	}
...
class i4_key_matchup_class
{
	struct command_matchup
	{
		...
		command_matchup(w16 command_id, int remap) :
			command_id(command_id),
			remap_id(remap) {
		}
