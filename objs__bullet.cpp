...
{    
  ...
  pitch=lpitch = i4_atan2(-vel.z, (float)sqrt(vel.x * vel.x +
                                       vel.y * vel.y));
  lroll=roll   = 0;
...
void g1_bullet_class::draw(g1_draw_context_class *context)
{
  ...
  else
  {
    ...
    g1_render.render_sprite(t_pos, tex, 0.05f, 0.05f);
  }
}
...
void g1_bullet_class::done(i4_bool hit, g1_object_class *hit_object)
{
  ...
  else if (hit)
  {
    ...
    if (flak)
      flak->setup(i4_3d_vector(x,y,h), 0.1f);
  }
