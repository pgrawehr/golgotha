...
public:
	...
	w8 is_transformed()
	{
		return (w8)get_flag(TRANSFORMED);
	}
	void set_is_transformed(w8 yes_no)
	{
		set_flag(TRANSFORMED, yes_no);
	}

	w8 is_projected()
	{
		return (w8)get_flag(PROJECTED);
	}
	void set_is_projected(w8 yes_no)
	{
		set_flag(PROJECTED, yes_no);
	}

	w8 is_selected()
	{
		return (w8)get_flag(SELECTED);
	}
	void set_is_selected(w8 yes_no)
	{
		set_flag(SELECTED, yes_no);
	}

	w8 need_undo()
	{
		return (w8)get_flag(NEED_UNDO_SAVE);
	}
	void set_need_undo(w8 yes_no)
	{
		set_flag(NEED_UNDO_SAVE, yes_no);
	}

	w8 is_clipped()
	{
		return (w8)get_flag(CLIP_CODE_CALCULATED);
	}
	void set_is_clipped(w8 yes_no)
	{
		set_flag(CLIP_CODE_CALCULATED, yes_no);
	}

	w8 is_w_calculated()
	{
		return (w8)get_flag(W_CALCULATED);
	}
	void set_is_w_calculated(w8 yes_no)
	{
		set_flag(W_CALCULATED, yes_no);
	}
