...
class i4_get_load_dialog : public i4_color_window_class
{
...
//don't use references here, as local stays in memory while caller will not.
  i4_get_load_dialog(const i4_const_str _title_name,
  ...
  add_child(5,maxh+2,text_win);
  maxh=maxh+2+text_win->height();
...
void receive_event(i4_event *ev)
 ...
 if (ev->type()==i4_event::USER_MESSAGE)
     ...
     if (uev->sub_type==OK)          // tell who-ever what the user typed
         ...
         filepick->update(items.size(),&items[0]);
         ...
         add_drives_to_list(items);
         dirpick->update(items.size(),&items[0]);
...
void i4_create_file_open_dialog(i4_graphical_style_class *style,
                                ...
                                w32 ok_id, w32 cancel_id)
    {
    i4_get_load_dialog *dlg=new i4_get_load_dialog(
    ...
    re=new i4_event_reaction_class(dlg, 
        new i4_user_message_event_class(i4_get_load_dialog::CLOSE));
