...
#define SPIN_ACCEL 0.02f
#define ATTACK_SPIN 0.8f
...
public:
		...
		: g1_map_piece_class(id,fp)
	{
		...
		set_flag(BLOCKING      |
				 //SELECTABLE    |  //what for? to give a target?
				 TARGETABLE    |
				 GROUND        |
				 HIT_GROUND    |
				 HIT_AERIAL    |
	...
	//{{{
	{
		...
		if (charge>0)
		{
			...
			if (bolt[3].valid())
			{
				bolt[3]->setup_look(0.8f, 0.05f, 0.05f, 0xff88ff, 0x8800ff, 1.0, 1.0, i4_T);
		...
		for (int i=0; i<3; i++)
		{
			...
			if (bolt[i].valid())
			{
				bolt[i]->setup_look(0.2f, 0.02f, (charge>0) ? 0.05f : 0.02f, 0xffffff, 0xff88ff);
	...
	//{{{
	{
		...
		else
		{
			...
			if (guard_time<=0)
			{
				...
				guard_pitch = -g1_float_rand(3) *i4_pi()*0.45f;
				i4_normalize_angle(guard_pitch);
				guard_time = 20;
			}
