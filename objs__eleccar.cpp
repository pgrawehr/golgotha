...
void g1_electric_car_init()
{
	...
	wheel_attach.set(0,0.1f,0.03f);
	wheel_offset.set(0,0.1f,0.03f);
	gun_r_attach.set(0.0422f,0,-0.0494f);
	gun_l_attach.set(0.0422f,0,0.0494f);
	gun_offset.set(0.0422f,0,0.0494f);
...
void g1_electric_car_class::skipload(g1_loader_class * fp)
{
	...
	fp->check_version(DATA_VERSION);
	fp->seek(fp->tell()+(NUM_ARC_POINTS*12*4)+1+(18*4));
	fp->end_version(I4_LF);
}
...
void g1_electric_car_class::draw(g1_draw_context_class * context, i4_3d_vector& viewer_position)
{
	...
	if (firing && health>0)
	{
		...
		g1_render.add_translucent_trail(context->transform,pts,
										...
										0.02f,0.02f,1,1,0xFFFFFF,0x0088FF);
										   ...
										   const i4_3d_vector &target)
{
	...
	for (i=0; i<NUM_ARC_POINTS; i++)
	{
		i4_float sin_factor = (float)sin(i4_pi()*i/(NUM_ARC_POINTS-1));

		//ARC #1
		rx = (i4_rand()&0xFFFF)/((i4_float)0xffff) * 0.8f - 0.2f;
		ry = (i4_rand()&0xFFFF)/((i4_float)0xffff) * 0.8f - 0.2f;
		rz = (i4_rand()&0xFFFF)/((i4_float)0xffff) * 0.8f - 0.2f;
		...
		map_point_height = g1_get_map()->map_height(px,py,pz) + 0.05f;
...
void g1_electric_car_class::post_think()
{
	...
	if (target)
	{
		...
		{
			fire();
			gun_right->rotation.x += 0.4f;
			gun_left->rotation.x  -= 0.4f;
		}
		else
		{
