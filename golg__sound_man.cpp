...
static i4_signal_object m_hmutex("sound_man_lock",1,1);
...
/*  //Does not work, as the dsound interface is already released when this gets executed => BOOOUUUM
   class g1_music_stream_uniniter: public i4_init_class
   	{
   	public:
   		virtual int init_type()
   			{
   			return I4_INIT_TYPE_FILE_MANAGER;//Must uninit before dsound gets released
   			}
   		virtual void uninit()
   			{
   ...
   g1_music_stream_uniniter_instance;
 */
...
void g1_sound_manager_class::poll(i4_bool game_is_running)
{
	...
	if (!i4_sound_man->is_enabled())
	{
		return;
	}
	/*if (WaitForSingleObject(m_hmutex,50)==WAIT_TIMEOUT)
	   	{
	   	return;//Nicht pollen, es ist gerade eine Operation im Gange.
	   	}
	 */
	if (!m_hmutex.wait_signal(50))
	{
		return;
	}
	...
	if (game_is_running)
	{
		if (!g1_music_stream) // no more music, go to the next song
		{
			if (music_count_down)
			{
				// this prevents trying to open a missing music file quickly
				music_count_down--;
			}
			else
			{
				...
				if (music)
				{
					...
					{
						//m_hmutex.signal();
						restart_song(li_get_string(cur_song,0),i4_F);
						//m_hmutex.wait_signal();
					}
		...
		for (g1_sfx_obj_class *s=sfx_obj_list; s; s=s->next_sfx)
		{
			if (s->stream)
			{
				float d=(float)s->max_hearable_distance;
			...
			if (s->stream)
			{
				float d=(float)sqrt(s->dist_from_camera_sqrd);
...
class g1_sound_man_updater :
	public i4_idle_class
{
public:
	virtual void idle()
	{
		...
		}                                  // update sound effects (play next narative sfx)
	}
