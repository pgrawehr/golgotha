...
{    
  ...
  r1_vert      v[3];
  i4_3d_vector ip,op;
  i4_float offs = i4_float(shrapnel_time)*(1.0f-t_grad_width)/MAX_SHRAPNEL_TIME;
  ...
  {        
    ...
    if (!r1_calc_outcode(&v[0]) && !r1_calc_outcode(&v[1]))
    {
      ...
      const i4_float SIZE=0.01f;

      v[2].v = v[0].v;
      v[2].w = v[0].w;

      i4_float dir = (float)(((v[0].px>v[1].px) ^ (v[0].py>v[1].py))? 1 : -1);
                              ...
                              w32 _num_pieces, int type)
{
  ...
  for (int i=0;i<(int)num_shrapnel_pieces;i++,p++)
  {
    i4_float f=0.2f;
    i4_float
      rx = i4_float_rand() * f - f/2.0f,
      ry = i4_float_rand() * f - f/2.0f,
      rz = i4_float_rand() * f;  // - f/2.0;
    
    p->type=(w8)(g1_rand(39)&3);
