...
public:
	...
	i4_bool is_space() const
	{
		return (i4_bool)(ch==' ' || ch=='\r' || ch=='\n' || ch=='\t');
	}
	i4_bool is_slash() const
	{
		return (i4_bool)(ch=='/');
	}
	i4_bool is_backslash() const
	{
		return (i4_bool)(ch=='\\');
	}
	i4_bool is_period() const
	{
		return (i4_bool)(ch=='.');
	}
#ifndef _WINDOWS
	...
	i4_char to_upper() const;
#endif
	i4_bool operator==(const i4_char &b) const
	{
		return (ch == b.ch);
	}
	i4_bool operator!=(const i4_char &b) const
...
public:
	...
	/** The copy constructor */
	i4_const_str(const i4_const_str &str) :
		ptr(str.ptr),
		len(str.len)
	{
	}
...
public:
		...
		explicit iterator(const iterator p, int i) :
			node(p.node+i)
		{
		}
		...
		iterator operator+(const sw32 other)
		{
			return iterator(node+other);
		}
		iterator operator+=(const sw32 len)
		{
			node+=len;
			return *this;
		}
	...
	i4_bool operator==(const i4_const_str &other) const
	{
		...
		{
			return ::strncmp(other.ptr,ptr,len)==0;
		}
	}
...
public:
	class iterator :
		public i4_const_str::iterator
	{
protected:
public:
		iterator(const iterator &p) :
			i4_const_str::iterator(p)
		{
		}
		explicit iterator(const iterator p, int i) :
		...
		void set(i4_char ch)
		{
			*((char_type *)node)=(char_type)ch.value();
		}

		char_type& operator*()
		{
			return *((char_type *)node);
		}
	};
	i4_str() :
		i4_const_str(),
	...
	i4_str(const i4_str &str) :
		i4_const_str(0)
	{
		init_from_string(str, (w16)str.length());
	}
	i4_str(const i4_const_str &str) :
		i4_const_str(0)
	{
		init_from_string(str, (w16)str.length());
	}
	i4_str(const i4_const_str &str, w16 _len) :
		i4_const_str(0)
	{
...
public:
	...
	int init_type()
	{
		return I4_INIT_TYPE_STRING_MANAGER;
	}
	void init();
	void uninit();
