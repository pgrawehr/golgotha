...
class radtablescalcer:public i4_init_class
	{
	public:
		virtual void init()
			{
                                             ...
                                             g1_object_class*& hit) const
{
  ...
  if (x_right>(sw32)w-1)  x_right=w-1; 
  if (y_top<0)      y_top=0;     
  if (y_bottom>(sw32)h-1) y_bottom=h-1;
...
//code bellow mainly for old maps
  ...
  if (c->is_blocking() || block->is_blocked((w16)xl,(w16)yt,dir)) return 0;
 
  c = cell(xr,yt);

  if (c->is_blocking() || block->is_blocked((w16)xr,(w16)yt,dir)) return 0;  

  c = cell(xl,yb);

  if (c->is_blocking() || block->is_blocked((w16)xl,(w16)yb,dir)) return 0;
  
  c = cell(xr,yb);

  if (c->is_blocking() || block->is_blocked((w16)xr,(w16)yb,dir)) return 0;
