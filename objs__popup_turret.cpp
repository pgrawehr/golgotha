...
const i4_float POPUP_HEIGHT      = 0.4f;
const i4_float POPUP_SPEED       = 0.1f;
const i4_float POPDOWN_SPEED     = 0.05f;
const i4_float POPUP_ROTATESPEED = 0.3f;
...
void g1_popup_turret_class::load(g1_loader_class *fp)
{
	g1_map_piece_class::load(fp);
	fp->check_version(DATA_VERSION);
	fp->write_format("f",&mount->rotation.y);
	fp->end_version(I4_LF);
};

void g1_popup_turret_class::skipload(g1_loader_class *fp)
{
...
void g1_popup_turret_class::think()
{
	...
	if (attack_target.valid())
	{
		...
		else
		{
			...
			fire_pitch = i4_atan2(-d.z,(float)sqrt(d.x*d.x+d.y*d.y));
