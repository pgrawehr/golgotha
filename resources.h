...
public:
	...
	{
		//or we'll get into trouble if cleanup() is called before
		//load()
		use=0;
		build_context_help=0;
		images=0;
		for (int i=0; i<G1_TOTAL_CURSORS; i++)
		{
			big_cursors[i]=0;
			small_cursors[i]=0;
		}
	...
	enum radius_mode_type {
		VIEW_LOW, VIEW_MEDIUM, VIEW_FAR
	} radius_mode;
	i4_float view_radii[3]; // contains the three fields, near, medium, and far
	i4_float visual_radius()
	{
		return view_radii[radius_mode];
	}
