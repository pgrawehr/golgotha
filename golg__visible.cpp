...
//The following code is obsolete and doesn't compile any more
//after changing the cell() code to a template function
/*
 ...
 #define OCTANT(f1, f2, f3, f4, f5, i1, s1, r1, r2)              \
   ...
   {                                                             \
   	...
   	{ G1_STOP(cx-1,cy-1); }                                     \
   	else if (map->cell((w16)cx, (w16)cy-1)->is_blocking())                 \
   	{ G1_STOP(cx,cy-1); }                                       \
   	else if (map->cell((w16)cx-1, (w16)cy)->is_blocking())                 \
   	{ G1_STOP(cx-1, cy);  }                                     \
   }                                                             \
   else                                                          \
   {                                                             \
   	...
   	if (nr -= liconst)                                          \
   	{                                                           \
   	  S("test 2"); if (map->cell((w16)cx, (w16)cy)->is_blocking())                      \
   		{ G1_STOP(cx,cy);  }                                    \
   	} else if  (map->cell((w16)cx, (w16)cy)->is_blocking())                \
   	{ G1_STOP(cx,cy); }                                         \
   	else if (map->cell((w16)cx-1, (w16)cy-1)->is_blocking())               \
   	{ G1_STOP(cx-1,cy-1); }                                     \
   	else if (map->cell((w16)cx, (w16)cy-1)->is_blocking())                 \
   	{ G1_STOP(cx,cy-1); }                                       \
   	else if (map->cell((w16)cx-1, (w16)cy)->is_blocking())                 \
   ...
   if (x1==x2)
   {
   	...
   	while (y1!=y2)
   	{
   	  if (map->cell((w16)x1,(w16)y1)->is_blocking())
   	  {
   		hit_x=x1;
   		hit_y=y1;
   		y2=y1-add;
							...
							g1_map_class *map)
{
	...
	while (t)
	{
		...
		if (nx!=lx && map->cell((w16)nx, (w16)ly)->is_blocking())
		{
			x2=lx;
			y2=ly;
			hit_x=nx;
		...
		else if (ny!=ly && map->cell((w16)lx, (w16)ny)->is_blocking())
		{
			x2=lx;
			y2=ly;
			hit_x=lx;
		...
		else if ((nx!=lx && ny!=ly) && map->cell((w16)nx, (w16)ny)->is_blocking())
		{
			x2=lx;
			y2=ly;
			hit_x=nx;
