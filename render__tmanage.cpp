...
r1_texture_manager_class::r1_texture_manager_class(const i4_pal *pal)
  : pal(pal),
    registered_tnames(0,128),image_list(16,64),memory_images(0,64)
{
...
void r1_texture_manager_class::matchup_textures()
{
  ...
  for (i=0; i<registered_tnames.size(); ++i)
  {
    ...
    if (h)
      registered_tnames[i].handle = h;//this handle is NOT persistent
                                                    ...
                                                    const i4_const_str &local_dir)
{
  ...
  {  
    ...
    if (i>=(int)old_cache_header_num_entries) i_done = i4_T;
...
{  
  ...
  if (textures_loaded||entries||active)//The textures flag has no assured semantics at all
    return i4_F;
  active=i4_T;
