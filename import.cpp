...
public:
	...
	{
		return _T("Imports Golgotha MODel files");
	}
	const TCHAR *ShortDesc()
	{
		return _T("Golgotha MODel files");
	}
	...
	{
		return _T("Copyright 1998 Crack dot Com");
	}
...
public:
	...
	{
		return new m1_import_class;
	}
	const TCHAR *ClassName()
	{
		return _T("Crack Importer");
	}
...
ClassDesc *GetCrackImportDesc()
{
	return &m1_import_descriptor;
}

class m1_loader :
	public g1_base_object_loader_class
//{{{
{
public:
	ImpInterface * im;
	...
	{
		return (g1_quad_object_class *)0xfaced;
	}
	...
	virtual void finish_object()
	{
		...
		if (!node)
		{
			g1_base_object_loader_class::obj=0;
			return;
		}
	...
	int load(const TCHAR * _name, ImpInterface * _im)
	{
		...
		dbg("Opening %s\n", name);
		if (!(f = i4_open(name, I4_READ)))
		{
			return 0;
		}
		dbg("Opening Loader %s\n", name);
		if (!(loader = i4_open_save_file(f)))
		{
			return 0;
		}
		dbg("Creating Object\n");
		if (!(obj = CreateNewTriObject()))
		{
			return 0;
		}
		dbg("Getting Mesh\n");
