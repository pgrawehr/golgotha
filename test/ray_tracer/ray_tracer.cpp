...
//     float r[5]={0.5,   0.3,  0.2,   0.5 };
	...
	void move_cam(float x, float y, float z)
	{
		...
		i-=c;
		i*=x;
		t.inverse_transform(i4_3d_vector(0,1,0), j);
		j-=c;
		j*=y;
		t.inverse_transform(i4_3d_vector(0,0,1), k);
		k-=c;
		k*=z;
...
//       cpath.a.normalize();
	...
	void thread_render()
	{
		...
		do
		{
			...
			t.inverse_transform(i4_3d_vector(1,0,1), xi);
			xi-=center;
			t.inverse_transform(i4_3d_vector(0,1,1), yi);
			yi-=center;
			...
			if (!reset_render && !abort_render)
			{
				...
				{
					w32 colors[5]={
						0x7f0000, 0xff0000, 0x007f00, 0xff00, 0x7f7f00
					};
