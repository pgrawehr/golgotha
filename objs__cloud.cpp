...
public:
		...
		: g1_object_class(id,fp)
	{
		...
		if (!cloud_ref)
		{
			...
			if (!cloud_im)
			{
				i4_error("couldn't load cload.tga");
			}
		}
	...
	void apply_shadow()
	{
		...
		for (int vy=0; vy<=mh; vy++)
		{
			...
			{
				float x1=interp((float)(d[0]&0xff), (float)(d[1]&0xff), rx);
				float x2=interp((float)(d[iw]&0xff), (float)(d[iw+1]&0xff), rx);
