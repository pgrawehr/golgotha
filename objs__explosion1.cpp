...
//this class mainly exists because we want to have a damage map
//for the explosion of units that is different from the object's
//damage map
...
g1_object_definer<g1_inv_explosion_class>
g1_inv_explosion_def("inv_explosion",0);
...
void g1_explosion1_class::draw(g1_draw_context_class *context)
{
  if (model_id && exp_frame<num_exp_frames)
  {
    ...
    i4_float texture_scale = 10*(float)sin(angle);
...
void g1_explosion1_class::setup(i4_float sx, i4_float sy, i4_float sh, w32 type)
{
      ...
      (sw32)sy>=0 && (sw32)sy<g1_get_map()->height())
  {
    ...
    if (type==HIT_OBJECT)
    {
      ...
      p.grow_speed=0.05f;
      p.max_speed=0.02f;
      p.air_friction=0.90f;
      p.particle_lifetime = 10;
      p.start_size_random_range=0.1f;

      p.num_create_attempts_per_tick=1;
      p.creation_probability=1;
      p.gravity=g1_resources.gravity*-0.25f;
    ...
    else if (type==HIT_GROUND)
    {
      ...
      p.start_size=0.02f;
      p.grow_speed=0.0005f;
      p.max_speed=0.2f;
      p.air_friction=0.70f;
      p.particle_lifetime=15;
      p.num_create_attempts_per_tick=1;
      p.creation_probability=0.4f;
