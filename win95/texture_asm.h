...
void draw_scanline_white_pc(edgeGrad * left, edgeGrad * right)
{
	...
	_asm {
		...
		mov ebx,left                      ;
		mov esi,r1_software_pixel_on      ;
		grab screen pointer
		mov edx,right                     ;
		mov eax,[ebx] edgeGrad.X           ;
		left x
		mov ecx,[edx] edgeGrad.X           ;
		edx = right x

			  sub ecx,eax                       ;
		edx = width
			  jle whtp_Return                   ;
		no pixels to draw, get out

		mov     [pTex],ecx                    ;
		just for a temp so it can be fild 'ed
...
//this is also needed if pixel_on.add isn' t used
		...
		edx = spanwidth
			  ...
			  shr edx,3                         ;
		ecx = width / subdivision length
			  and     eax,7                         ;
		eax = width mod subdivision length
    ...
    //the dword ptrs below shut up compiler warnings (though they don' t do qwords)
				...
				;
			calculate right side OverZ terms    ;
			st0 st1 st2 st3 st4 st5 st6 st7
			fadd dword ptr grads.dOneOverZdX8  ;
			1/ZR U/ZL V/ZL UL VL
			fxch    st(1)                         ;
			U/ZL 1/ZR V/ZL UL VL
			fadd dword ptr grads.dUOverZdX8    ;
			U/ZR 1/ZR V/ZL UL VL
			fxch    st(2)                         ;
			V/ZL 1/ZR U/ZR UL VL
			fadd dword ptr grads.dVOverZdX8    ;
			V/ZR 1/ZR U/ZR UL VL
			...
			fld1                                ;
			1    V/ZR 1/ZR U/ZR UL VL
			;
			@todo overlap this guy
			fdiv st,st(2)                    ;
			ZR V/ZR 1/ZR U/ZR UL VL
			fld st                          ;
			ZR ZR V/ZR 1/ZR U/ZR UL VL
			fmul st,st(2)                    ;
			VR ZR V/ZR 1/ZR U/ZR UL VL
			fxch    st(1)                       ;
			ZR VR V/ZR 1/ZR U/ZR UL VL
			fmul st,st(4)                    ;
			UR VR V/ZR 1/ZR U/ZR UL VL

			test edx,edx                     ;
			check for any full spans
			jz whtp_HandleLeftoverPixels

			whtp_SpanLoop :

			;
			at this point the FPU contains      ;
			st0 st1 st2 st3 st4 st5 st6 st7
			;
			UR VR V/ZR 1/ZR U/ZR UL VL
			...
			fld     st(5)                         ;
			UL UR VR V/ZR 1/ZR U/ZR UL VL
			fmul    [FixedScale]                  ;
			UL8 UR VR V/ZR 1/ZR U/ZR UL VL
			fistp   [UFixed]                      ;
			UR VR V/ZR 1/ZR U/ZR UL VL

			fld     st(6)                         ;
			VL UR VR V/ZR 1/ZR U/ZR UL VL
			fmul    [FixedScale]                  ;
			VL8 UR VR V/ZR 1/ZR U/ZR UL VL
			fistp   [VFixed]                      ;
			UR VR V/ZR 1/ZR U/ZR UL VL
			...
			fsubr   st(5),st                      ;
			UR VR V/ZR 1/ZR U/ZR dU VL
			fxch    st(1)                         ;
			VR UR V/ZR 1/ZR U/ZR dU VL
			fsubr   st(6),st                      ;
			VR UR V/ZR 1/ZR U/ZR dU dV
			fxch    st(6)                         ;
			dV UR V/ZR 1/ZR U/ZR dU VR

			fmul    [FixedScale8]                 ;
			dV8 UR V/ZR 1/ZR U/ZR dU VR
			fistp   [DeltaV]                      ;
			UR V/ZR 1/ZR U/ZR dU VR

			fxch    st(4)                         ;
			dU V/ZR 1/ZR U/ZR UR VR
			fmul    [FixedScale8]                 ;
			dU8 V/ZR 1/ZR U/ZR UR VR
			fistp   [DeltaU]                      ;
			V/ZR 1/ZR U/ZR UR VR

			;
			increment terms for next span       ;
			st0 st1 st2 st3 st4 st5 st6 st7
			;
			Right terms become Left terms---->  ;
			V/ZL 1/ZL U/ZL UL VL

			fadd dword ptr grads.dVOverZdX8              ;
			V/ZR 1/ZL U/ZL UL VL
			fxch    st(1)                         ;
			1/ZL V/ZR U/ZL UL VL
			fadd dword ptr grads.dOneOverZdX8            ;
			1/ZR V/ZR U/ZL UL VL
			fxch    st(2)                         ;
			U/ZL V/ZR 1/ZR UL VL
			fadd dword ptr grads.dUOverZdX8              ;
			U/ZR V/ZR 1/ZR UL VL
			fxch    st(2)                         ;
			1/ZR V/ZR U/ZR UL VL
			fxch    st(1)                         ;
			V/ZR 1/ZR U/ZR UL VL
			...
			add esi,ebx
			mov ebx,1
			mov     [4+UVintVfracStepV],esi       ;
			non V carry slot
whtp_twidth_log2_patch1:
			shl ebx,3                         ;
			this is to get the texwidth
			add esi,ebx
			mov     [UVintVfracStepV],esi         ;
			V carry slot
			...
			mov esi,[UFixed]                  ;
			get u 16.16 fixedpoint coordinate
			add esi,dword ptr grads.dUdXModifier
			mov ecx,[VFixed]                  ;
			and v... before ebp gets pushed
...
whtp_twidth_log2_patch2:
			...
			adc esi,[4+UVintVfracStepV+ebx*4]   ;
			U pipe only
			mov bl,dh

			mov bh,dl
			mov eax,dword ptr[cTable+8000 h+eax*4]
			...
			One cycle stall here U exp flow i think
...
whtp_HandleLeftoverPixels:
			...
			st0 st1 st2 st3 st4 st5 st6 st7
			;
			inv.means invalid numbers        ;
			inv.inv.inv.inv.inv.UL VL

			cmp     [WidthModLength],0          ;
			are there remaining pixels to draw ?
			jz whtp_FPUReturn              ;
			nope, pop the FPU and bail

			mov ebx,right                   ;
			get right edge pointer
			...
			fld     st(5)                       ;
			UL inv.inv.inv.inv.inv.UL VL
			fmul    [FixedScale]                ;
			UL8 inv.inv.inv.inv.inv.UL VL
			fistp   [UFixed]                    ;
			inv.inv.inv.inv.inv.UL VL

			fld     st(6)                       ;
			VL inv.inv.inv.inv.inv.UL VL
			fmul    [FixedScale]                ;
			VL8 inv.inv.inv.inv.inv.UL VL
			fistp   [VFixed]                    ;
			inv.inv.inv.inv.inv.UL VL

			cmp     [WidthModLength],1          ;
			calc how many steps to take
			jz whtp_OnePixelSpan           ;
			just one, don 't do deltas
    ...
    ; @todo rearrange things so we don' t need these two instructions
			...
			fld     [ebx] edgeGrad.VOverZ        ;
			V/Zr inv.inv.inv.UL VL
			fsub dword ptr grads.dVOverZdX   ;
			V/ZR inv.inv.inv.UL VL
			fld     [ebx] edgeGrad.UOverZ        ;
			U/Zr V/ZR inv.inv.inv.UL VL
			fsub dword ptr grads.dUOverZdX   ;
			U/ZR V/ZR inv.inv.inv.UL VL
			fld     [ebx] edgeGrad.OneOverZ      ;
			1/Zr U/ZR V/ZR inv.inv.inv.UL VL
			fsub dword ptr grads.dOneOverZdX ;
			1/ZR U/ZR V/ZR inv.inv.inv.UL VL

			fdivr   [One]                       ;
			ZR U/ZR V/ZR inv.inv.inv.UL VL

			fmul    st(1),st                    ;
			ZR UR V/ZR inv.inv.inv.UL VL
			fmulp   st(2),st                    ;
			UR VR inv.inv.inv.UL VL
			...
			fsubr   st(5),st                    ;
			UR VR inv.inv.inv.dU VL
			fxch    st(1)                       ;
			VR UR inv.inv.inv.dU VL
			fsubr   st(6),st                    ;
			VR UR inv.inv.inv.dU dV
			fxch    st(6)                       ;
			dV UR inv.inv.inv.dU VR

			fidiv   [WidthModLength]            ;
			dv UR inv.inv.inv.dU VR
			fmul    [FixedScale]                ;
			dv8 UR inv.inv.inv.dU VR
			fistp   [DeltaV]                    ;
			UR inv.inv.inv.dU VR

			fxch    st(4)                       ;
			dU inv.inv.inv.UR VR
			fidiv   [WidthModLength]            ;
			du inv.inv.inv.UR VR
			fmul    [FixedScale]                ;
			du8 inv.inv.inv.UR VR
			fistp   [DeltaU]                    ;
			inv.inv.inv.UR VR
			...
			fld     st(1)                       ;
			inv.inv.inv.inv.UR VR
			fld     st(2)                       ;
			inv.inv.inv.inv.inv.UR VR
