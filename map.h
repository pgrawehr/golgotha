...
enum {
	...
	//! Selected Vertices on the map.
	//! Not part of G1_MAP_ALL because G1_MAP_VERTS encompases selected verts
	//! (if loading/saving data, G1_MAP_SELECTED_VERTS is ignored if G1_MAP_VERTS is present)
	G1_MAP_SELECTED_VERTS=(1<<31)
};
...
enum {
	//warning: must take care of intersections with G1_MAP* constants
...
public:
	...
	void mark_for_recalc(w32 flags)
	{
		recalc |= flags;
	}

	//! Gets the block map for the specified grade.
	g1_block_map_class *get_block_map(w8 grade) const
	{
		return const_cast<g1_block_map_class*>(&block[grade]);
	}
	...
	g1_astar_map_solver_class *get_astar_solver()
	{
		return astar_solver;
	}
	...
	g1_critical_map_maker_class *get_map_maker()
	{
		return &map_maker;
	};
...
public:
	...
	//! variables as input.
	template <typename T>
	g1_map_cell_class *cell(T x, T y) const
	{
		return cells + ((w32)y)*w + ((w32)x);
	}
 ...
 */
	...
	g1_critical_graph_class *get_critical_graph()
	{
