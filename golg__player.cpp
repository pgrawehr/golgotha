...
int &g1_player_info_class::num_points()
{
	return get_vars()->get(li_num_points);
}
int &g1_player_info_class::num_stank_deaths()
{
	return get_vars()->get(li_num_stank_deaths);
}
int &g1_player_info_class::num_stank_lives()
{
	return get_vars()->get(li_num_stank_lives);
}


int &g1_player_info_class::money()
{
	return get_vars()->get(li_money);
}
int &g1_player_info_class::income_rate()
{
	return get_vars()->get(li_income_rate);
}
float &g1_player_info_class::damage_multiplier()
{
	return get_vars()->get(li_damage_multiplier);
}
...
void g1_player_info_class::calc_upgrade_level()
{
	...
	while (l)
	{
		...
		if (required_number<=points)
		{
			// do we have enough kills for this level
...
void g1_player_info_class::set_ai(g1_team_api_class *_ai)
{
	...
	if (_ai==0) //uninit in progess
	{
		if (ai!=g1_human)
		{
			//still not delete human interface, might be in use
			delete ai;
		}
		ai=0;
		return;
...
i4_bool g1_player_info_class::load(g1_loader_class *fp)
{
	...
	if (!ai)
	{
		i4_error("INTERNAL: no ai in player_info load");
	}
	ai->init();
...
void g1_player_manager_class::set_ai(w32 player_num, const char *name)
{
	...
	{
		list[player_num]->set_ai(g1_create_ai(name));
	}                                                //deletes old one itself
}
