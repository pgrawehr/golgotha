...
{
public:
	name_table(int entries = 20, int grow = 100) :
		table<char *>(entries,grow)
	{
	}

	name_table(name_table &other) :
		table<char *>(20,200)
	{
		add_table(other);
	}
	...
	name_table& operator=(const name_table &tab)
	{
		clear();
		add_table(tab);
		return *this;
