...
i4_window_class::i4_window_class(w16 w, w16 h) :
	w(w),
	h(h)
{
	last_x=mouse_x=-10000;
...
i4_parent_window_class::i4_parent_window_class(w16 w, w16 h) :
	i4_window_class(w,h)
{
		...
		case i4_event::WINDOW_MESSAGE:
			{
				...
				switch (mess->sub_type)
				{
					...
					case i4_window_message_class::REQUEST_NEXT_KEY_FOCUS:
						next_key_focus();
						break;
					case i4_window_message_class::REQUEST_LEFT_KEY_FOCUS:
						left_key_focus();
						break;
					case i4_window_message_class::REQUEST_RIGHT_KEY_FOCUS:
						right_key_focus();
						break;
					case i4_window_message_class::REQUEST_UP_KEY_FOCUS:
						up_key_focus();
						break;
					case i4_window_message_class::REQUEST_DOWN_KEY_FOCUS:
						down_key_focus();
						break;
										  ...
										  i4_bool propogate_to_children)
{
	...
	x1+=x();
	y1+=y();             // transform to global coordinates
	x2+=x();
	y2+=y();
...
void i4_window_manager_class::receive_event(i4_event *ev)
{
	if (ev->type()==i4_event::WINDOW_MESSAGE)
	{
		...
		if (cc->sub_type==i4_window_message_class::CHANGE_CURSOR)
		{
			...
			{
				// make sure we have default cursor first
...
public:
	...
	char *name()
	{
		return "MWM_STYLE_IMPLEMENTATOR";
	}
...
class i4_mwm_close_button_class :
	public i4_window_class
{
protected:
...
class i4_mwm_passive_close_button_class :
	public i4_mwm_close_button_class
{
...
public:
	...
	i4_parent_window_class *user_window()
	{
		return user_area;
	}
...
class i4_mwm_modal_window_class :
	public i4_mwm_window_class
{
public:
...
i4_draggable_window_class::i4_draggable_window_class(w16 w, w16 h) :
