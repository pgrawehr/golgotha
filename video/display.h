...
public:
  ...
  {name=0;driver_id=0;priority=0;display=0;next=0;}
...
enum i4_change_type { I4_NOTIFY_ONLY, //accept as new framebuffer size. Usage is dangerous!
					  I4_ADJUST_FRAMEBUFFER_ONLY, //adjust only the framebuffer size (i.e the window has already its new size)
					  I4_CHANGE_RESOLUTION }; //change everything
...
public:
  ...
  // bad in a game because the 3d pipeline must be flushed before this can happen.
  ...
  // change_mode notifies the driver that the resolution should be changed
  // The third parameter defines wheter the actual display already has changed
  // (i.e the user dragged the window) or that it should now change.
  virtual i4_bool change_mode(w16 newwidth,w16 newheight, w32 newbitdepth, i4_change_type change_type)=0;
