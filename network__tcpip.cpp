...
class i4_tcpip_address :
	public i4_net_address
{
public:
	sockaddr_in addr;
...
class i4_tcpip_finder :
	public i4_finder_socket
{
	int poll_fd, listen_fd, listen_port, poll_port;
	int t;
...
public:
	...
	int setup(int poll_port, int listen_port)
	{
		...
		{
			i4_warning("Couldn't get my own IP");
			return 0;
		}
		...
		if (poll_fd<0)
		{
			i4_warning("out of sockets creating the poll socket");
			return 0;
		}
		...
		{
			i4_warning("setsockopt broadcast failed");
			return 0;
		}
		...
		int maxmsgsize=0;
		int maxmsgsize_size=sizeof(maxmsgsize);

#ifdef _WINDOWS
		if (getsockopt(poll_fd,SOL_SOCKET, SO_MAX_MSG_SIZE, (char *)&maxmsgsize, &maxmsgsize_size)!=0)
		{
			i4_warning("The maximum packet size could not be determined. Assuming it works anyway.");
		}


		i4_warning("The maximum packet size for udp on this implementation is %i bytes.",maxmsgsize);
		if (maxmsgsize<MAX_PACKET_SIZE)
		{
			i4_error("ERROR: The maximum packet size for udp is less than %i bytes on this system. Cannot continue.",MAX_PACKET_SIZE);
			return 0;
		}
#endif
	...
	void connect_to(w32 ipaddr) //currently assumes ipaddr has big-endian format already
	{
		...
		{
			host.sin_addr.s_addr = ipaddr; //htonl(ipaddr);
		}

		if (connect(poll_fd, (sockaddr *) &host, sizeof(host))==-1)
		{
			i4_warning("connect to this host failed");
			return;
		}
...
class i4_tcpip_notifier :
	public i4_notifier_socket
{
	int fd;
	w8 reply_packet[MAX_PACKET_SIZE];
public:
	int er;
