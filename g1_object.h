...
public:
	...
	g1_object_chain_class *next_non_solid()
	{
		return next;
	}
};
...
public:
	...
	virtual w32 private_safe_to_cast() const
	{
		return 0;
	}
	...
	enum {
		SAVE_FLAGS= SELECTED | THINKING
	};                                     //This mask is applied on object creation from a file only

	//! Where all the flags go.
	w32 flags;
	//! Some convenience functions to modify flags.
	int get_flag(int x) const
	{
		return flags & x;
	}
	void set_flag(int x, int value)
	{
		if (value)
		{
			flags|=x;
	...
	i4_bool selected() const
	{
		return (w8)flags & SELECTED;
	}
...
public:
	...
	virtual ~g1_object_definition_class()
	{
		g1_remove_object_type(type);
	}
	...
	i4_bool editor_selectable()
	{
		return (w8)flags & EDITOR_SELECTABLE;
	}
