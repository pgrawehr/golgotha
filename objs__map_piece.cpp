...
i4_float position_table[battle_step] = {
...
void g1_map_piece_class::skipload(g1_loader_class * fp)
{
	...
	if (t)
	{
		...
		{
			g1_id_ref::skip(fp);
		}                       //just needs an g1_id_ref to do this
	}
	fp->end_version(I4_LF);
}
...
//	  !get_flag(SPECIALTARGETS))))
	...
	if (r<=0.01 && !get_flag(SPECIALTARGETS))
	{
		return;
	}

	x_left   = i4_f_to_i(x-r);
	if (x_left<0)
	{
		x_left=0;
	}
	x_right  = i4_f_to_i(x+r);
...
void g1_map_piece_class::advance_path()
{
	...
	else
	{
		...
		{
			return;
		}         //we are already unlinked and/or have entered a formation
										 ...
										 i4_bool reversible)
{
		...
		||(!(solveparams&SF_OK) && prefered_solver()))
	{
		...
		if (path_info||ctrl)
		{
			...
			{
				go_reverse=i4_F;
			}                                //if vehicle cannot move backwards
											 ...
											 i4_3d_vector &d)
{
	...
	if (dest_theta<0&&dtheta>0)
	{
		dtheta=0;
	}                                    //don't flip sign of dtheta
	...
	if (faroff)    //we must calculate the way on the actual position.
	{
		...
		if (next_path.valid())
		{
			...
			{
				dist=speed/4;
			}                    //to be shure caller calls advance_path() soon
			path_cos *= dist_w;
			path_sin *= dist_w;
			path_tan_phi *= dist_w;
			//if (path_tan_phi>0.05)
			...
			for (int i=0; i<path->total_links(team); i++)
			{
				...
				if (mp && mp!=this)
				{
					...
					else
					{
						...
						dist += mp->path_pos;
						if (path!=mp->prev_object.get())
						{
							//must be the last on that link
							continue;
						}
						//other=mp->link_on();
