...
WPARAM win32_input_class::translate_windows_key(WPARAM wkey, w32 &m)
{
	...
	switch (wkey)
	{
		...
		case VK_LCONTROL:
			key=I4_CTRL_L;
			m=I4_MODIFIER_CTRL_L;
			break;
		case VK_RCONTROL:
			key=I4_CTRL_R;
			m=I4_MODIFIER_CTRL_R;
			break;
		case VK_MENU:
		case VK_LMENU:
			key=I4_ALT_L;
		...
		case VK_LSHIFT:
			key=I4_SHIFT_L;
			m=I4_MODIFIER_SHIFT_L;
			break;
		case VK_RSHIFT:
			key=I4_SHIFT_R;
			m=I4_MODIFIER_SHIFT_R;
			break;
		case VK_CAPITAL:
			key=I4_CAPS;
			break;
   ...
   Newer seen an international keyboard?
		...
		default:
			...
			else
			{
				...
				key = 0;
			}   //if it's something else, wait for the WM_CHAR message
			break;
	}
	return key;
}
											 ...
											 mouse_button_state state)
{
	...
	if (state==DOWN)
	{
		...
		if (m)
		{
			...
			//last_down[type]=now;
			if (mouse_x<m->x()||mouse_y<m->y())
			{
				return;
			}
			if ((mouse_x>m->x()+m->width())||(mouse_y>m->y()+m->height()))
			{
				return;
			}
		}
