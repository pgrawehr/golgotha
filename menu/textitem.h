...
class i4_checkbox_images_class: public i4_init_class
	{
	...
	public:
	i4_checkbox_images_class(){checkbox=0;radiobox=0;};
	virtual void init();
	virtual void uninit();
	virtual int init_type(){return I4_INIT_TYPE_AFTER_ALL;}//must init after image-loaders
	};

class i4_checkbox_class:public i4_text_item_class
	{
	protected:
	...
	virtual i4_bool set_state(w32 newstate)
		...
		if ((state>2) || (((flags&THREESTATES)==0)&&(state>1)))
			return i4_F;//Invalid state for current setting.
		state=newstate;
		request_redraw();
		return i4_T;
		}
