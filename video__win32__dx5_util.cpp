...
{        
  ...
  {
	if (front_surface==primary_surface) front_surface=0;
    if (primary_surface) primary_surface->Release();
	primary_surface=0;
...
//  ddsd.dwSize=sizeof(ddsd);
  if (surface3&&(surface3->IsLost()==DDERR_SURFACELOST)) surface3->Restore();
/*  if (surface3) 
	  {
