...
#ifndef __MATRIX_FULL4x4_HPP_
#define __MATRIX_FULL4x4_HPP_

#include <string.h>
#include "math/point.h"

//this class partially implements full 4x4 matrix operations.
//Preferably use i4_transform_class instead (faster, more complete)
...
public:
	...
	//{{{
	{
		i4_float cos_ax=(float)cos(ax);
		i4_float sin_ax=(float)sin(ax);
	...
	//{{{
	{
		i4_float cos_ay=(float)cos(ay);
		i4_float sin_ay=(float)sin(ay);
	...
	//{{{
	{
		i4_float cos_az=(float)cos(az);
		i4_float sin_az=(float)sin(az);
