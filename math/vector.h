...
public:
	...
	i4_vector3_template(const i4_vector3_template& p) :
		x(p.x),
		y(p.y),
		z(p.z) {
	}
	i4_vector3_template(Coord _x,Coord _y,Coord _z) :
		x(_x),
		y(_y),
		z(_z) {
	}
	...
	Coord CosAngle(const i4_vector3_template& b) const
	{
		Coord temp;

		temp=this->dot(b)/(length()*b.length());
	}
...
public:
	...
	i4_2d_vector(i4_float x, i4_float y) :
		x(x),
		y(y) {
	}

	i4_float length() const {
		return (float)sqrt(x*x+y*y);
	}
	i4_float dot(const i4_2d_vector &b)  {
		return x*b.x + y*b.y;
	}
	i4_float perp_dot(const i4_2d_vector &b)  {
		return x*b.y-y*b.x;
	}

	i4_2d_vector& operator-=(const i4_2d_vector& b) {
		x-=b.x;
		y-=b.y;
		return *this;
	}
	...
	i4_2d_vector operator+(const i4_2d_vector& b) {
		return i4_2d_vector(x+b.x, y+b.y);
	}
	i4_2d_vector operator-(const i4_2d_vector& b) {
		return i4_2d_vector(x-b.x, y-b.y);
	}
