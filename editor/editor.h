...
protected:
	...
	void open_anim();
	void open_anim_ok(i4_user_message_event_class * ev);
	...
	struct {
		i4_text_input_class * w, * h;
		int orient;
	} resize_dialog;
	struct {
		i4_text_input_class * amount;
	} terrain_noise_dialog;
	struct {
		i4_text_input_class * w, * h, * name;
	} new_dialog;


	struct {
		i4_text_input_class * name;
	} sky_dialog;
...
public:
	...
	void do_undo();
	void do_redo();
	void save_as(const i4_const_str &fname);
	i4_bool have_map()
	{
		return g1_map_is_loaded();
	}
	...
	void redraw_all();
	const i4_const_str &get_editor_string(char * res_name)
	{
		return e_strs.get(res_name);
	}
	i4_const_str *get_editor_array(char * res_name)
	{
		return e_strs.get_array(res_name);
	}
