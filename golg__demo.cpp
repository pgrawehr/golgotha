...
static li_object *play_demo(li_object * o, li_environment * env)
{
	...
	if (!i4_get_status(demo_name,fstat))
	{
		return 0;
	}
	...
	//i4_file_open_message_class opm(G1_SAVEGAME_LOAD_OK, new i4_str(demo_name));
	//i4_current_app->receive_event(&opm);

	if (g1_load_level(demo_name)) //there's actually little that could have
	//failed till here. (except corrupt data)
	{
...
li_automatic_add_function(quit,"quit");
//"quit" is called explicitly from lisp-code if a fatal error occurs.
//Not to be called if user chooses File/Quit or ALT-F4!
