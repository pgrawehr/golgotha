...
class m1_animation_dialog_class :
...
public:
	...
	~m1_animation_dialog_class()
	{
		delete caption[SPEED];
		delete input[SPEED];
		delete caption[DU];
		delete input[DU];
		delete caption[DV];
		...
		delete caption[MAXY];
		delete input[MAXY];
		delete caption[MAXFRAMES];
		delete input[MAXFRAMES];
		delete caption[NONE];
		for (int i=0; i<ITEMS; i++)
		{
			caption[i]=0;
			...
			case i4_event::OBJECT_MESSAGE:
				{
					...
					{
						case SPEED:
							special->speed = p.read_float();
							break;
						case DU:
							special->du = p.read_float();
							break;
						case DV:
							special->dv = p.read_float();
							break;
						case MAXFRAMES:
							special->max_frames = p.read_number();
							if (special->max_frames<1)
							{
								special->max_frames=1;
							}
							break;
						case MAXX:
							special->frames_x = p.read_number();
							if (special->frames_x<1)
							{
								special->frames_x=1;
							}
...
li_automatic_add_function(m1_update_special, "update_edit_special");
