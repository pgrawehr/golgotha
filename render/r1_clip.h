...
struct r1_clip_vert_array
{
	...
	r1_clip_vert_array(int buf_size, r1_vert *buf, int total) :
		total(total),
		buf_size(buf_size),
		buf(buf) {
		;
...
inline w8 r1_calc_outcode(i4_3d_vector &v)
{
	...
	if (v.z<r1_near_clip_z)
	{
		clip_code |=16;
	}
	else if (v.z>r1_far_clip_z)
	{
		clip_code |=32;
	}
...
inline w8 r1_calc_outcode(r1_vert * v)
{
	...
	if (v->v.z<r1_near_clip_z)
	{
		clip_code |=16;
	}
	else if (v->v.z>r1_far_clip_z)
	{
		clip_code |=32;
	}
