...
public:
	w32                     get_render_device_flags()
	{
		return render_device_flags;
	}
	r1_shading_type         get_shade_mode()
	{
		return shade_mode;
	}
	r1_alpha_type           get_alpha_mode()
	{
		return alpha_mode;
	}
	r1_write_mask_type      get_write_mask()
	{
		return write_mask;
	}
	w32                     get_constant_color()
	{
		return const_color;
	}
	r1_filter_type          get_filter_mode()
	{
		return filter_mode;
	}
	i4_bool states_have_changed;
	r1_render_api_class() :
		tmanagers(0,64)                 //i4_mem_manager has not been initialized->don't allocate mem now
	{
		tmanager=0;
...
//Call this method after the render window gets a DISPLAY_CHANGE notification
	...
	r1_texture_manager_class *get_tmanager()
	{
		return tmanager;
	}                                                            // created by init()
...
public:
	...
	virtual i4_bool is_feature_on(w32 feature_bits)
	{
		return i4_F;
	}
...
/// Inverts the z value.
/// Used for projection.
inline i4_float r1_ooz(i4_float z)
{
	return 0.9999f/z;
}
