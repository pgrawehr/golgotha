...
#include "render/software/r1_software_types.h"
//#include <ddraw.h>

class r1_software_class :
	public r1_render_api_class
{
public:
	char *name()
	{
		return "Software Span Buffer Renderer (unknown subclass)";
	}
	...
	//PG: These are important! A renderer may only draw inside
	//a render window (a special case of window which is transparent
	//for hardware rendering)
	r1_render_window_class *create_render_window(int w, int h, r1_expand_type expand_type)=0;
	...
	//virtual r1_texture_manager_class *install_new_tmanager(r1_texture_manager_class** old_tman);
	r1_texture_manager_class *install_new_tmanager(w32 &index);
	void use_texture(w32 tman_index, r1_texture_handle material_ref,
...
protected:
	...
	sw32 total_texture_mem()
	{
		return 1024*1024*8;
	}
	sw32 texture_mem_start()
	{
		return 0;
	}
	...
	//(assumed) format of the software render buffer
	//This is the format one thinks, the render buffer has, if it's not
	//the same as the above one (so far, the software renderer always assumes
	//16 bit) conversions are necessary.
	i4_pixel_format rbuf_fmt;
	i4_bool holy_mode;
...
class r1_software_render_window_class :
