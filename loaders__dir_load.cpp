    ...
    sections(512,512)
{
  ...
  if (use_buffer)//should be false if _in points to the stack (i.e for ram_files)
	  {
  ...
  if (sig==i4_check_sum32("GOLG",4))
    use_16=1;
  else if (sig==i4_check_sum32("GOLG_SECTION_ID=32",17)) //The identification of .gmod and .level - files (0x1E63FAF9)
    use_16=0;
  else if (sig==i4_check_sum32("GOLGRAWDATA",11))//A network packet has this id
	  {
	  use_16=0;
	  return;
	  }
...
i4_bool i4_loader_class::goto_section(w32 section_id)
{
  ...
  {
    i4_warning("goto section : non-linear %d", section_id);//This is nothing serious
...
{  
  ...
  if (data_size==65535)//the realtime saver writes a -1 because the
	  {//data length is not known at save time
	  i4_check_end=-1;
	  return;
	  }
  data_size -= 4;
...
{
  if (in->tell()!=i4_check_end&&(i4_check_end!=-1))//the size reads always as -1 from network
