...
class r1_opengl_render_window_class :
...
public:
	...
	virtual void clear_area(int x1, int y1, int x2, int y2, w32 color, float z)
	{
		...
		// These methods clear the entire buffer, they may be used for debugging
		// purposes, if problems with the z-buffer occur, or similar. But
		// they shan't be in there in the end, since that causes false behaviour
		// of the user interface window clipping system.
	...
	virtual void render_lines(int t_lines, r1_vert *verts)
	{
		...
		//here to get the number of verts to pass.
		pass_verticies(t_lines+1,verts);
		glEnd();
		OGL_ERROR_CHECK();
	}
