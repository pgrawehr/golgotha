...
class g1_add_camera_funs_initer :
...
void g1_check_position(float &x, float &y)
{
	...
	{
		x = (float)(g1_get_map()->width()-1);
	}                                          // JJ Cast
	if (x+1>=g1_get_map()->height())
	{
		x= (float)(g1_get_map()->height()-1);
	}                                          // JJ Cast
}

void g1_view_state_class::use_next_cam()
{
	if (next_cam.camera_at.get()==current_cam.camera_at.get())
	{
		// don't snap if same camera position
...
void g1_view_state_class::update_watch_mode()
{
	...
	if (!current_cam.follow_object.get())
	{
		// object following is gone
		next=i4_T;
	}
...
void g1_view_state_class::update_strategy_mode()
{
	...
	else if (end.gx>g1_map_width-2.0f)
	{
		end.gx = g1_map_width-2.0f;
	}
	if (end.gy<2.0)
	{
		end.gy = 2.0;
	}
	else if (end.gy>g1_map_height-2.0f)
	{
		end.gy = g1_map_height-2.0f;
	}
