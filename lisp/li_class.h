...
public:
	...
	char *string_value(int member) const
	{
		return ((li_string *)values[member])->value();
	}
	int &int_value(int member)
	{
		return *(((int *) (values+member)));
	}
	float &float_value(int member)
	{
		return *(((float *) (values+member)));
	}
	...
	char *get(li_string_class_member &c)
	{
		return string_value(member_offset(c, LI_STRING));
	}
	int &get(li_int_class_member &c)
	{
		return int_value(member_offset(c, LI_INT));
	}
	float &get(li_float_class_member &c)
	{
		return float_value(member_offset(c, LI_FLOAT));
	}
	li_symbol *&get(li_symbol_class_member &c)
	{
		return symbol_value(member_offset(c, LI_SYMBOL));
	}
	li_object *&get(li_class_member &c)
	{
	...
	li_object *get_member(li_class_member &c)
	{
		return value(c.name);
	}
