...
public:
	enum {
		GC_FLAG=1<<30
	};                    //ICC seems not to be sure wheter this should be
	//signed or unsigned
	i4_bool is_marked() const
	{
		if (_type & GC_FLAG)
		{
			return i4_T;
	...
	void mark_free()
	{
		_type=LI_INVALID_TYPE;
	}

	li_object(li_type_number type)
	{
		_type=type;
	}
	li_type_number type() const
	{
		return _type;
	}
	li_type_number unmarked_type() const
	{
...
public:
	struct symbol_data
	{
		...
		symbol_data(li_string *name,w32 flags) {
			_value=0;
			_fun=0;
			_name=name;
			left=0;
...
public:
	...
	{
		return data->_fun;
	}                                          //return function [use li_get_fun(sym,env) instead]
	li_object *value() const
	{
		return data->_value;
	}                                              //return value [use li_get_val(sym,env) instead]
	li_string *name() const
	{
		return data->_name;
	}
...
public:
	li_function(li_function_type fun)  :
		li_object(LI_FUNCTION),
		_fun(fun)
	{
		_minargs=0xffff;
		_maxargs=0xffff;
	};
	li_function_type value()
	{
		return _fun;
	}
...
public:
	...
	li_object *bindings()
	{
		return _data->_bindings;
	};
	li_environment *locals()
	{
		return _data->_locals;
	};
	li_string *name()
	{
		return _data->_name;
	};
	struct user_function_data *data()
	{
		return _data;
	};
	void cleanup()
	...
	public li_object
{
	li_type_number _x;
public:
	li_type(li_type_number x) :
		li_object(LI_TYPE)
	{
		_x=x;
	}
	li_type_number value()
	{
		return _x;
	}
...
public:
	...
	li_vector() :
		li_object(LI_VECTOR)            //create an object
	{
	...
	li_object *data()
	{
		return _vector->operator [](0);
	};                                                     //for compatiblity only: returns first entry
	li_object *remove(int l)
	{
		_vector->remove(l);
		return this;
	};
