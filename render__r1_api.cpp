...
r1_render_api_class *r1_create_api(i4_display_class *for_display, char *name)
{
  ...
  for (r=r1_render_api_class::first; r; r=r->next)
	  {
	  if(r->render_device_flags==R1_SOFTWARE)
		  softrenderers.add(r);
	  }
  for (r=r1_render_api_class::first; r; r=r->next)
  {
    cont=i4_F;
    for (int i=0;i<softrenderers.size();i++)
		{
		if (softrenderers[i]==r) 
			cont=i4_T;
		}
	if (cont)
		continue;
    ...
    {
      //if(r->render_device_flags != R1_SOFTWARE) 
		  // JJ dont want software get first hardware
		  //Changed and fixed by PG
      //{
  ...
  // NO HARDWARE GET FIRST // ADDED BY JJ  
  for (r=r1_render_api_class::first; r; r=r->next)
  {
        
    // r->init.. for LINEAR texturing ??? JJ
    if ((!name || strcmp(r->name(), name)==0) && r->init(for_display))
    {
        r1_render_api_class_instance = r;
        return r;
...
{
  if (r) r->uninit();
  if (r1_render_api_class_instance==r) 
	  {
	  //delete r1_render_api_class_instance;
	  r1_render_api_class_instance = 0;
	  }
}
...
i4_bool r1_render_api_class::resize(w16 newx, w16 newy)
	{
	return i4_T;
	}

i4_bool r1_render_api_class::redepth(w16 new_bitdepth)
	{
	return i4_T;
	}
...
void r1_render_api_class::uninit()
{
  ...
  for(int i=0;i<tmanagers.size();i++)
	  {
	  delete tmanagers[i];
	  }
  tmanagers.uninit();
...
void r1_render_api_class::delete_tmanager(w32 index)
	{
	delete tmanagers[index];//doesn't delete the entry, as the index must be persistent
	tmanagers[index]=0;
	}
...
void r1_render_api_class::clear_area(int x1, int y1, int x2, int y2, w32 color, float z)
{
  ...
  v[0].px=(float)x1;     v[0].py=(float)y1;      v[0].w=w;   v[0].v.z=z;
  v[1].px=(float)x1;     v[1].py=(float)y2+1;    v[1].w=w;   v[1].v.z=z;
  v[2].px=(float)x2+1;   v[2].py=(float)y2+1;    v[2].w=w;   v[2].v.z=z;
  v[3].px=(float)x2+1;   v[3].py=(float)y1;      v[3].w=w;   v[3].v.z=z;
...
i4_float r1_near_clip_z = 0.001f;
