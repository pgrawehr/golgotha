...
{  
  ...
  i4_dx5_check(r1_software_render_surface->Lock(NULL,&ddsd,DDLOCK_WAIT | DDLOCK_NOSYSLOCK,0));
...
{
  if (!r1_software_render_buffer_is_locked) return;
  i4_dx5_check(r1_software_render_surface->Unlock(NULL));
...
i4_bool r1_software_dd_class::init(i4_display_class *display)
	...
	r1_software_render_surface = dx5_common.create_surface(DX5_SURFACE, wi,hi, 0);  
	...
	if (ddpf.dwRGBBitCount!=(w16)rbuf_fmt.pixel_depth)
		{
...
i4_bool r1_software_gdi_class::init(i4_display_class *display)
	...
	if (!r1_software_class::init(display))
		return i4_F;
	if (render_image) //this should not have been initialized already...
		delete render_image;
...
//    case R1_COPY_1x1_SCANLINE_SKIP:
    if (lock_check) r1_software_dd_unlock_render_buffer();    
    ...
    if (lock_check) r1_software_dd_lock_render_buffer();
//    break;
//  }
...
void r1_software_gdi_class::clear_area(int x1, int y1, int x2, int y2, w32 color, float z)
  ...
  if (write_mask & R1_COMPARE_W)   // need to use spans to do this
  {
    ...
    v[3].px=(float)x1;   v[3].py=(float)y1;    v[3].v.z = z; v[3].w = 1/z;
    v[2].px=(float)x2+1; v[2].py=(float)y1;    v[2].v.z = z; v[2].w = 1/z;
    v[1].px=(float)x2+1; v[1].py=(float)y2+1;  v[1].v.z = z; v[1].w = 1/z;
    v[0].px=(float)x1;   v[0].py=(float)y2+1;  v[0].v.z = z; v[0].w = 1/z;
...
class r1_software_win_uniniter_class: public i4_init_class
	{
	public:
