...
static i4_str *m1_locate_texture(const i4_const_str &texture_filename)
{
	...
	//We KNOW that this one exists, if .tga doesn't. Must not open it for decompression
							...
							const i4_const_str &out_ctext)
{
	...
	if (src_texture->get_pal()->source.pixel_depth==I4_32BIT)
	{
		...
		//generate the mips
		result = r1_write_tga_mips(src_texture,dst_ctext,0,texname,G1_CHROMA_COLOR);
