...
S1_SFX(firing, "misc/missile_in_flight_22khz.wav", S1_3D, 5);
...
void g1_tank_buster_init()
{
	...
	missile_attach.set(-0.1984f,0,0.1468f);
...
void g1_tank_buster_class::load(g1_loader_class * fp)
{
	...
	fp->read_format("1fff",&rack_state,&missile->rotation.x,
					&missile->rotation.y,&missile->rotation.z);
	fp->end_version(I4_LF);
}
...
void g1_tank_buster_class::think()
{
	...
	switch (rack_state)
	{
		...
		case RAISING:
			if (i4_rotate_to(missile->rotation.y,-i4_pi()/8 + i4_2pi(),0.1f)!=0.0)
			{
				keep_going = i4_T;
			}
		...
		case LOWERING:
			if (i4_rotate_to(missile->rotation.y,0,0.1f)!=0.0)
			{
				keep_going = i4_T;
			}

			if (i4_rotate_to(missile->rotation.z,0,0.1f)!=0.0)
			{
				keep_going = i4_T;
			}
