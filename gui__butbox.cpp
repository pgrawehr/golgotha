											   ...
											   i4_menu_item_class::reaction_type type)
{
	...
	else if (type==i4_menu_item_class::DEPRESSED)
	{
		if (but==current_down)
		{
			((i4_checkbox_class *)current_down)->set_state(1);
		}                                                 // sorry we need you to stay down
	}
}
					  ...
					  va_list &ap)
{
	...
	if (i.get().value()=='%')
	{
		...
		{
			i4_error("expecting p or d after %");
		}
	}
	else
	{
...
void i4_slider_class::send_change()
{
	if (notify)
	{
		...
		{
			i4_kernel.send_event(notify, &ev);
		}
	}
}
...
w32 i4_input_box(const i4_const_str &title, const i4_const_str &message, const i4_const_str &preset, i4_str &rettext, w32 flags)
{
	...
	{
		w=150;
	}              //avoid that the input-box gets to small.
...
void i4_tab_bar::parent_draw(i4_draw_context_class &context)
{
	...
	for (int i=0; i<data->tabs.size(); i++)
	{
		...
		if (i-1!=cur)
		{
			im->bar(x1, y1+2, x1, y2, bright, context);
		}                                            // left edge
...
public:
	...
	{
		return new cursor_blink_class(object);
	}
} ;
...

{
	...
	if (max_width<(int)default_string.length())
	{
		// need a string at least as long as default
...
void i4_text_input_class::become_active()
{
	...
	if (!cursor_on)
	{
		// make sure cursor is not draw when we are not in focus
		cursor_on=i4_T;
	}
	request_blink();
}
...
void i4_text_input_class::become_unactive()
{
	...
	if (lastcontext!=-1)
	{
		i4_key_man.set_context(lastcontext);
	}
	//lastcontext=-1;
	i4_kernel.unrequest_events(this,i4_device_class::FLAG_CHAR_SEND);
	if (cursor_on)
...
void i4_text_scroll_window_class::output_char(const i4_char &ch)
{
	...
	if (!style->font_hint || !style->font_hint->normal_font)
	{
		return;
	}
	i4_font_class * fnt=style->font_hint->normal_font;
