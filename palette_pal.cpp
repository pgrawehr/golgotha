...
i4_lookup_pal::i4_lookup_pal(i4_pixel_format *source_fmt)
{
	...
	int size=0;
	source.lookup=0;
	if (8==source.pixel_depth)
	{
		size=256*4;
	}
	else if (4==source.pixel_depth)
	{
		size=16*4;
	}                                       //Rarelly used modes.
	else if (2==source.pixel_depth)
	{
		size=2*4;
	}
	if (size>0)
	...
	for (i=pal_list.begin(); i!=pal_list.end(); ++i)
	{
		if (format->pixel_depth == i->source.pixel_depth)
		{
				...
				format->pixel_depth == I4_2BIT)
			{
				...
				if (8==format->pixel_depth)
				{
					size=256*4;
				}
				else if (4==format->pixel_depth)
				{
					size=16*4;
				}                              //Rarelly used modes.
				else if (2==format->pixel_depth)
				{
					size=2*4;
				}
