...
//int i4_lk_cmp_size[]={0,0,0,256*4, 16*4, 2*4};
...
i4_lookup_pal::i4_lookup_pal(i4_pixel_format *source_fmt)
{
  ...
  //Calculate Size of Palette Memory (above doesn't work anymore,
  //cause we changed the values of the pixel_depth enum)
  int size=0;
  source.lookup=0;
  if (8==source.pixel_depth) size=256*4;
  else if (4==source.pixel_depth) size=16*4;//Rarelly used modes.
  else if (2==source.pixel_depth) size=2*4;
  if (size>0)
	  {
...
i4_pal *i4_pal_manager_class::register_pal(i4_pixel_format *format)
{
  ...
  for (i=pal_list.begin(); i!=pal_list.end(); ++i)
  {
    if (format->pixel_depth == i->source.pixel_depth)
    {
  	...
  	if (8==format->pixel_depth) size=256*4;
  	else if (4==format->pixel_depth) size=16*4;//Rarelly used modes.
  	else if (2==format->pixel_depth) size=2*4;
...
void i4_pal_manager_class::init()
{
  ...
  d.pixel_depth=I4_24BIT;
  def_na_24=register_pal(&d);
