...
void i4_saver_class::start_version(w16 version)
{
	...
	{
		out->write_16(version);
		out->write_16(0xffff); //unknown, but not needed
	}
}
...
void i4_saver_class::end_version()
{
	if (state==DIRECTORY_CREATE)
	{
		skips.add((w16)(current_offset-last_version_start));
	}
}
...
i4_bool i4_saver_class::begin_data_write()
{
	...
	if (out->write_32(signature)!=sizeof(w32))
	{
		return i4_F;
	}
	if (out->write_32(sections.size())!=sizeof(w32))
	{
		return i4_F;
	}
	...
	for (w32 i=0; i< (w32)sections.size(); i++)
	{
		if (out->write_32(sections[i].section_id)!=sizeof(w32))
		{
			return i4_F;
		}
		if (out->write_32(sections[i].section_offset + off)!=sizeof(w32))
		{
			return i4_F;
		}
	}
