...
static g1_team_icon_ref radar_imr("bitmaps/radar/rocket_tank.tga");
...
void g1_rocket_tank_class::fire()
{
	...
	if (target)
	{
		...
		btrans.transform(i4_3d_vector(0.1f, 0, 0), pos);
		btrans.transform(i4_3d_vector(1.0f, 0, 0), dir);
		dir-=pos;
...
void g1_rocket_tank_class::think()
{
	...
	if (attack_target.valid())
	{
		...
		{
			guard_pitch = -(dist - 9.0f) *i4_pi()/8/(25.0f-9.0f);
		}
		else
		{
			guard_pitch = 0;
		...
		dangle = i4_rotate_to(missile_rack->rotation.z, guard_angle, 0.1f);
		aimed = (dangle<0.1 && dangle>-0.1);
		dangle = i4_rotate_to(missile_rack->rotation.y, guard_pitch, 0.1f);
		aimed &= (dangle<0.1 && dangle>-0.1);
	...
	else
	{
		...
		i4_rotate_to(missile_rack->rotation.z, guard_angle, 0.1f);
		i4_rotate_to(missile_rack->rotation.y, guard_pitch, 0.1f);
	}
}
