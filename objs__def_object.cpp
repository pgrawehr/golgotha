...
static w32 g1_oflag_values[]={
	g1_object_class::THINKING,
...
static void read_vect(i4_3d_vector &v, li_object *o, li_environment *env)
{
	...
	o=li_cdr(o,env);
	v.y=(float)li_get_float(li_eval(li_car(o,env), env),env);
	o=li_cdr(o,env);
	...
	for (li_object *l=li_cdr(o,env); l; l=li_cdr(l,env))
	{
		...
		else if (sym==li_get_symbol("mini_object", s_mini_object))
		{
			...
			for (; prop; prop=li_cdr(prop,env)) // prop = ((offset 0 0 0.1) (position 0 0 0) (model_name "gunport_barrel"))
			{
				...
				{
					li_error(env,"USER: %O should be (offset/position/rotation x y z)",sym);
				}
			}
		...
		{
			int found=0;
			for (int i=0; i<G1_F_TOTAL-1; i++)
			{
				//cannot use can_attack with non-map-piece objs
				if (sym==s_ofun[i])
				{
