...
public:
								 ...
								 i4_status_class *status)
	{
		...
		if (bits==8 && planes==1)
		{
			...
			i4_pixel_format fmt;
			memset(&fmt,0,sizeof(i4_pixel_format)); //required, because some functions
			//test these bits before checking for the pixel_depth
			...
			if (compress==0)
			{
				...
				for (i=0; i<height; i++)
				{
					...
					if (end_of_line_skip)
					{
						// each row is 0-padded to a 4-byte boundary
