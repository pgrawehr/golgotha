...
#define MAX_PACKET_SIZE 2048 //the maximum size we use for udp
enum i4_protocol_type {
	I4_TCPIP
};                                    // this is the only kind we have support for right now

enum i4_socket_type {
	I4_PACKETED_DATA,                            // data is sent in packets (i.e. udp)
	I4_CONTINOUS_STREAM
};                                             // data is seen as a continous stream
...
inline i4_net_protocol *i4_get_first_protocol()
{
	return i4_net_protocol::first;
}
