						  ...
						  char * texture_name, w32 chroma_color)
{
	...
	{
		f=i4_open(i4_const_str(dst_file), I4_WRITE);
	}
	...
	{
		delete f;
	}                  //delete f only if we opened it ourselves.
	return i4_T;
}
	...
	while (fn.filename[i])
	{
		...
		switch (fn.filename[i])
		{
			...
			case 'f':
				...
				break;
			default:
				i4_warning("invalid filename, no valid id");
		}
		i++;
...
w32 mip_r_and;
w8 mip_r_shift;                //r and g shifts are to the left
w32 mip_g_and;
w8 mip_g_shift;
w32 mip_b_and;
w8 mip_b_shift;                //b shift is to the right

//chroma-key shifting info
w32 mip_c_r_and;
w8 mip_c_r_shift;                  //r and g shifts are to the left
w32 mip_c_g_and;
w8 mip_c_g_shift;
w32 mip_c_b_and;
w8 mip_c_b_shift;                  //b shift is to the right
w32 mip_c_a_and;
w8 mip_c_a_shift;                  //a shift is to the left

//alpha texture shifting info
w32 mip_a_r_and;
w8 mip_a_r_shift;                  //r and g shifts are to the left
w32 mip_a_g_and;
w8 mip_a_g_shift;
w32 mip_a_b_and;
w8 mip_a_b_shift;                  //b shift is to the right
w32 mip_a_a_and;
w8 mip_a_a_shift;                  //a shift is to the left
