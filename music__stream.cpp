										   ...
										   i4_bool _3d_capable)
{
	...
	if (i4_load_wav_info(fp, info))
	{
		...
		if (voice)
		{
			...
			if (locked_buffer_size==0) //idirectsound::lock failed
			{
...
void i4_stream_wav_player::pause_all()
{
	for (int i=0; i<self_references.size(); i++)
	{
		self_references[i]->pause();
	}

}

void i4_stream_wav_player::unpause_all()
{
	for (int i=0; i<self_references.size(); i++)
	{
		self_references[i]->unpause();
	}

}

class i4_stream_wav_self_reference_unloader :
...
public:
	virtual void uninit()
	{
		...
		{
			i4_warning("Warning: Stream wav player references leaked.");
		}
...
LI_HEADER(pause_sounds) {
	i4_stream_wav_player::pause_all();
	i4_thread_sleep(500);
	return li_nil;
}

LI_HEADER(resume_sounds) {
	i4_stream_wav_player::unpause_all();
	i4_thread_sleep(400);
	return li_nil;
}

li_automatic_add_function(li_pause_sounds,"pause_sounds");
li_automatic_add_function(li_resume_sounds,"resume_sounds");
...
i4_bool i4_stream_wav_player::poll()
{
	...
	if (wait_read)
	{
		// if we are waiting for a read to finish or init failed
		return i4_T;
	}
...
i4_stream_wav_player::~i4_stream_wav_player()
{
	for (int i=0; i<self_references.size(); i++)
	{
		if (self_references[i]==this)
		{
			self_references.remove(i);
			break;
		}
	}
	if (voice)
