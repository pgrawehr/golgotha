...
public:
  ...
  void reset() { head=tail=0; data.uninit(); data.set_used(); }

  i4_dynamic_que():data(SIZE,GROW) { reset();sort_fun=0;}

  i4_dynamic_que(compare_type s_fun):data(SIZE,GROW) {reset();sort_fun=s_fun;};

  ~i4_dynamic_que(){/*data.uninit();*/}//the compiler should add this automatically

  i4_bool empty() { return (i4_bool)(head==tail); }

  //i4_bool full() { return (i4_bool)((head==SIZE-1 && tail==0) || head+1==tail); }
  i4_bool full() {return i4_F;}; //implication on a dynamic queue: cannot get full
