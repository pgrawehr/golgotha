...
void g1_quad_object_class::calc_extents()
{
	...
	for(i=0; i<num_animations; i++)
	{
		for(j=0; j<animation[i].num_frames; j++)
		{
			...
			{
				if(animation[i].vertex[k].v.length()>extent)
				{
					extent=animation[i].vertex[k].v.length();
				}
				if(animation[i].vertex[k].v.x < minx)
				{
					minx=animation[i].vertex[k].v.x;
				}
				if(animation[i].vertex[k].v.y < miny)
				{
					miny=animation[i].vertex[k].v.y;
				}
				if(animation[i].vertex[k].v.z < minz)
				{
					minz=animation[i].vertex[k].v.z;
				}
				if(animation[i].vertex[k].v.x > maxx)
				{
					maxx=animation[i].vertex[k].v.x;
				}
				if(animation[i].vertex[k].v.y > maxy)
				{
					maxy=animation[i].vertex[k].v.y;
				}
				if(animation[i].vertex[k].v.z > maxz)
				{
					maxz=animation[i].vertex[k].v.z;
				}
			}
		}
	}
...
g1_quad_object_class *g1_base_object_loader_class::load(i4_loader_class *fp)
{
	...
	if (nv<highest_ref)
	{
		if (i4_error("WARNING: g1_base_object_loader_class::load: Reference to high vertex number found. GMOD file damaged?")==2)
		{
			//IDCANCEL
			return 0;
		}
