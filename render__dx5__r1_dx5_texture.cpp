...
i4_profile_class pf_dx5_make_surfaces("dx5 surface alloc");
i4_profile_class pf_jpg_texture_decompress("decompressing jpg");
...
void r1_dx5_texture_class::init()
{
	...
	else
	{
		...
		if ((allowed_texture_formats&R1_MIPFLAGS_USE24)!=0)
		{
			memcpy(&regular_format,&reg24_format,sizeof(regular_format));
		}                                                                //this must exist
		else
		{
...
#define R1_DX5_SURFACE_24BIT        8
#define R1_DX5_SURFACE_32BIT       16
...
IDirectDrawSurface3 *dx5_make_surface(sw32 w, sw32 h, w8 flags)
{
	...
	{
		ddsd.ddsCaps.dwCaps = DDSCAPS_TEXTURE | /*DDSCAPS_SYSTEMMEMORY|*/ DDSCAPS_3DDEVICE;
	}
	...
	/*
	   ...
	   if (flags & R1_DX5_SURFACE_24BIT)
	   	ddsd.ddpfPixelFormat=dd_fmt_888;
	   else if (flags &R1_DX5_SURFACE_32BIT)
	...
	{
		ddsd.ddpfPixelFormat=dd_fmt_888;
	}                                 //cannot have chroma or alpha
	else if (flags & R1_DX5_SURFACE_32BIT)
	{
		if ((flags & R1_DX5_SURFACE_HOLY) || (flags & R1_DX5_SURFACE_ALPHATEXTURE))
		{
			ddsd.ddpfPixelFormat=dd_fmt_8888;
		}
		else
	...
	else if (flags & R1_DX5_SURFACE_HOLY)
	{
		ddsd.ddpfPixelFormat=dd_fmt_1555;
	}
	else if (flags & R1_DX5_SURFACE_ALPHATEXTURE)
	{
		ddsd.ddpfPixelFormat=dd_fmt_4444;
	}
	else
	...
	i4_dx5_check(dx5_common.ddraw->GetAvailableVidMem(&memcaps,&memtotal,&memfree));
	...
	{
//#ifdef _DEBUG
//    i4_warning("r1_dx5_texture::dx5_make_surface failed. Possibly out of Video Memory");
//#endif

		i4_dx5_check(res); //Did we really get DDERR_OUTOFVIDMEM? If not, we have a real problem
		...
		//ddsd.ddsCaps.dwCaps|=DDSCAPS_SYSTEMMEMORY;
		res=dx5_common.ddraw->CreateSurface(&ddsd,&return_surface,0);
		if (res)
		{
			i4_dx5_check(res); //The only error that is actually possible is DDERR_OUTOFMEMORY
			return 0;
		}
	}
	...
	w8 node_alloc_flags)
{
	...
	{
		tex_by=3;
		create_flags=R1_DX5_SURFACE_24BIT;
	}
	if (load_info->flags&R1_MIPFLAGS_FORCE32)
	{
		tex_by=4;
		create_flags|=R1_DX5_SURFACE_32BIT;
	}
	...
	if (!vram_surface3)
	{
		if (release_higher_miplevels(this))
		{
			//attempts to release any higher
			//miplevels of other tmans. Neither releases any local textures
			//nor the lowest miplevel of a texture
			vram_surface3 = dx5_make_surface(actual_w, actual_h, create_flags);
		}
	}
	...
	else
	{
		...
		if (!fp)
		{
			...
			load_texture_from_file(load_info->texture_name,
								   ...
								   mip->width,mip->height,ddsd.lPitch,tex_by,mip->entry->is_transparent(),
...
i4_bool r1_dx5_texture_class::async_mip_load(r1_mip_load_info * load_info)
{
	...
	if (load_info->src_file)
	{
		...
		new_used->data     = new w8[mip->width*mip->height*tex_by]; //(w8 *)I4_MALLOC(mip->width*mip->height*2,"dx5 async load alloc");
	...
	else
	{
		...
		{
			//gtx-files
			new_used->data     = new w8[mip->width*mip->height*2]; //(w8 *)I4_MALLOC(mip->width*mip->height*2,"dx5 async load alloc");
...
void r1_dx5_texture_class::free_mip(void * vram_handle)
{
	...
	if (u->vram_surface)
	{
		//if (u->vram_surface->IsLost()==DDERR_SURFACELOST)
														 ...
														 int frame_counter, int desired_width)
{
	...
	u->vram_surface->Lock(0,&ddsd,DDLOCK_WAIT|DDLOCK_NOSYSLOCK,0);
	memcpy(im->data,ddsd.lpSurface,act_w*act_h*tex_by);
...
int r1_dx5_texture_class::set_texture_image(r1_texture_handle handle, i4_image_class * im)
{
	...
	for (int i=0; i<memory_images.size(); i++)
	{
		if (memory_images[i].id==tid)
		{
			...
			u->vram_surface->Lock(0,&ddsd,DDLOCK_WAIT|DDLOCK_NOSYSLOCK|DDLOCK_WRITEONLY,0);
			size_image_to_texture(ddsd.lpSurface,im,act_w,act_h,tex_by,
...
void r1_dx5_texture_class::next_frame()
{
	...
	for (i=0; i<finished_array.size(); i++)
	{
		...
		i4_dx5_check(u->vram_surface->Lock(0, &ddsd, DDLOCK_WAIT | DDLOCK_WRITEONLY | DDLOCK_NOSYSLOCK, 0));
		...
		i4_dx5_check(vram_texture->GetHandle(r1_dx5_class_instance.d3d_device, &u->texture_handle));
