...
#include "pch.h"
#include "path_api.h"
#include "solvemap_astar.h" //(OLI) Debug hack
#include <math.h>
...
static int sp_floor(i4_float val)
{
	return i4_f_to_i(val);
}
static int sp_ceil(i4_float val)
{
	return 2048-i4_f_to_i(2048.0f-val);
}
										   ...
										   i4_float rad_x, i4_float rad_y) const
{
	...
	if (y1+rad_y>i4_float(height()))
	{
		rad_y = i4_float(height()) - y1;
	}
	if (y2-rad_y<0)
	{
		rad_y = y2;
	}
	if (y2+rad_y>i4_float(height()))
	{
		rad_y = i4_float(height()) - y2;
	}
	if (x1-rad_x<0)
	{
		rad_x = x1;
	}
	if (x1+rad_x>i4_float(width()))
	{
		rad_x = i4_float(width()) - x1;
	}
	if (x2-rad_x<0)
	{
		rad_x = x2;
	}
	if (x2+rad_x>i4_float(width()))
	{
		rad_x = i4_float(width()) - x2;
	}
	...
	iy = sp_floor(y1-rad_y);                       // integer 'pixel' location
	y = (float)iy;
	ey = sp_ceil(y2+rad_y);                        // ending row
	x = x1 + (i4_float(iy+1)-y1)*dx;            // center of edge
	if (dx>0)
	{
		lbx = sp_floor(x1-rad_x);                  // bounding edges
		rbx = sp_ceil(x2+rad_x);
		lx = lbx;                               // initial left edge
		rx = sp_ceil(x1+rad_x);                    // initial right edge
		max_fit = rad_x + rad_y*dx;             // initial corridor width
	}
	else
	{
		lbx = sp_floor(x2-rad_x);
		rbx = sp_ceil(x1+rad_x);
		lx = sp_floor(x1-rad_x);
		rx = rbx;
	...
	while (iy<ey && max_fit>0)
	{
		...
		nlx = sp_floor(x - max_fit);
		nlx = (lbx>nlx) ? lbx : nlx;
		nrx = sp_ceil(x + max_fit);
		nrx = (rbx<nrx) ? rbx : nrx;
...
//#endif
	...
	i4_float d = (float) sqrt(1.0+dx*dx);
	return max_fit/d;
}
#else
