	...
	while (x!=xs || y!=ys)
	{
		...
		switch (solve_dir(x,y))
		{
			case G1_NORTH:
				y++;
				break;
			case G1_NORTH|G1_EAST:
				x++;
				y++;
				break;
			case G1_NORTH|G1_WEST:
				x--;
				y++;
				break;
			case G1_SOUTH|G1_WEST:
				x--;
				y--;
				break;
			case G1_SOUTH|G1_EAST:
				x++;
				y--;
				break;
			case G1_SOUTH:
...
w32 g1_map_solver_class::unblocked(g1_block_map_class * block,i4_float startx, i4_float starty, i4_float destx, i4_float desty)
{
	...
	n=xc;
	m=yc;       // abs(.) is not needed, they can't be negative
